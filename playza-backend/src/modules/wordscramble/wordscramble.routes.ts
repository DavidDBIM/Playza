...
import { createRoom, joinRoom, getRoom, submitScore, findQuickMatch } from './wordscramble.service'
...
router.post('/room/:roomId/submit', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    const { score, rounds_won } = req.body
    if (score === undefined || rounds_won === undefined) {
      res.status(400).json({ success: false, message: 'score and rounds_won required' }); return
    }
    const data = await submitScore(req.params.roomId, req.user!.id, score, rounds_won)
    res.json({ success: true, data })
  } catch (err: any) {
    res.status(400).json({ success: false, message: err.message })
  }
