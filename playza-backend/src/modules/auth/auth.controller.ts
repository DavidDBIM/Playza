...
export async function verifyAdminMfaController(req: Request, res: Response) {
  try {
    ...
    // Set HttpOnly Cookie for the token (used by browser-based requests)
    ...
    // ALSO return access_token in the response body — the admin portal's
    // axios interceptor reads from localStorage (Bearer token pattern),
    // not from the httpOnly cookie, so the token must be in the response
    // for the frontend to store it and use it on subsequent requests.
    res.status(200).json({
      success: true,
      data: {
        user: result.user,
