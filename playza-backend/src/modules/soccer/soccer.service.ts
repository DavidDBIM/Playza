...
export async function createSoccerRoom(
  ...
  const { data, error } = await supabaseAdmin
    ...
    .insert({
      code,
      host_id: userId,
      guest_id: null,       // null until a real player joins
      stake,
      status: 'waiting',
      is_bot: false,
...
export async function getSoccerRoom(roomId: string, userId: string) {
  const { data, error } = await supabaseAdmin
    ...
    .single()
  if (error) throw error
  if (data.host_id !== userId && data.guest_id !== userId && !data.is_bot) {
...
export async function createBotRoom(
  ...
  // guest_id is NULL for bot rooms — no fake user ID needed
  const { data, error } = await supabaseAdmin
    .from('soccer_rooms')
    .insert({
      code,
      host_id: userId,
      guest_id: null,       // ← key fix: null, not a fake UUID
      stake,
      status: 'active',     // bot rooms start immediately
      is_bot: true,
...
export async function findQuickMatch(userId: string, stakeValue: number) {
  ...
  // No match found — create a new waiting room
...
export async function updateGameState(roomId: string, userId: string, gameState: any) {
  const { data: room } = await supabaseAdmin
    .from('soccer_rooms')
    .select('host_id, guest_id, is_bot')
    .eq('id', roomId)
    .single()
  if (!room) throw new Error('Room not found')
  // For bot rooms only host needs to be checked
  if (!room.is_bot && room.host_id !== userId && room.guest_id !== userId) {
    throw new Error('Not authorised')
  }
  if (room.is_bot && room.host_id !== userId) {
    throw new Error('Not authorised')
  }
...
export async function finishSoccerGame(roomId: string, userId: string, winnerId: string | null) {
  const { data: room } = await supabaseAdmin
    ...
    .single()
  if (!room) throw new Error('Room not found')
  if (room.host_id !== userId && room.guest_id !== userId && !room.is_bot) {
    throw new Error('Not authorised')
  }
...
export async function joinTournament(tournamentId: string, userId: string) {
  ...
  const { data, error } = await supabaseAdmin
    ...
    .update({
      participants,
      status: participants.length === t.size ? 'active' : 'waiting',
    })
    .eq('id', tournamentId)
    .select()
    .single()
...
export async function getActiveTournaments(userId: string) {
  const { data, error } = await supabaseAdmin
    .from('soccer_tournaments')
    .select(`id, name, size, stake, difficulty, status, participants, created_at,
      host:users!host_id(id, username, avatar_url)`)
