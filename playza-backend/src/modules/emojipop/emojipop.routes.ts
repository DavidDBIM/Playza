...
import { createRoom, joinRoom, getRoom, submitResult, findQuickMatch } from './emojipop.service'
...
router.post('/room/:roomId/submit', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (score === undefined) { res.status(400).json({ success: false, message: 'score is required' }); return }
    const data = await submitResult(req.params.roomId, req.user!.id, Number(score))
    res.json({ success: true, data })
  } catch (err: any) {
    res.status(400).json({ success: false, message: err.message })
  }
