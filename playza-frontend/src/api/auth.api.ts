...
export interface VerifyOtpResponse {
  ...
  data: {
    ...
    user: {
      id: string; email: string; username: string; phone: string;
      referral_code: string; is_email_verified: boolean; pza_points: number;
      avatar_url?: string; first_name?: string; last_name?: string;
    };
  };
}
...
export interface ForgotPasswordResponse { success: boolean; data: { message: string } }
export interface RefreshTokenResponse {
  success: boolean;
  data: { access_token: string; refresh_token: string };
}
export interface LogoutResponse { success: boolean; data: { message: string } }
export interface ResetPasswordPayload { access_token: string; new_password: string }
export interface ResetPasswordResponse { success: boolean; data: { message: string } }
...
export const verifyOtpApi = async (payload: VerifyOtpPayload): Promise<VerifyOtpResponse> => {
  const { data } = await axiosInstance.post<VerifyOtpResponse>("/auth/verify-otp", payload);
  return data;
};
export const resendOtpApi = async (payload: ResendOtpPayload): Promise<ResendOtpResponse> => {
  const { data } = await axiosInstance.post<ResendOtpResponse>("/auth/resend-otp", payload);
  return data;
};
...
export const forgotPasswordApi = async (payload: ForgotPasswordPayload): Promise<ForgotPasswordResponse> => {
  const { data } = await axiosInstance.post<ForgotPasswordResponse>("/auth/forgot-password", payload);
  return data;
};
export const refreshTokenApi = async (refreshToken: string): Promise<RefreshTokenResponse> => {
  const { data } = await axiosInstance.post<RefreshTokenResponse>("/auth/refresh", { refresh_token: refreshToken });
  return data;
};
...
export const resetPasswordApi = async (payload: ResetPasswordPayload): Promise<ResetPasswordResponse> => {
  const { data } = await axiosInstance.post<ResetPasswordResponse>("/auth/reset-password", payload);
  return data;
};