...
const GamePlay = () => {
    ...
    if (!game) {
        return (
            <div className="fixed inset-0 flex flex-col items-center justify-center bg-slate-950 text-white p-6 text-center">
                <h2 className="text-2xl font-black uppercase tracking-widest mb-4">Arena Error</h2>
                ...
                <button 
                    ...
                    className="bg-primary text-slate-950 px-8 py-3 rounded-xl font-black uppercase text-xs tracking-widest hover:scale-105 transition-all"
                >
                    Return to Lobby
                </button>
            </div>
    ...
    if (!game.iframeUrl) {
        return (
            <div className="fixed inset-0 flex flex-col items-center justify-center bg-slate-950 text-white p-6 text-center">
                <h2 className="text-2xl font-black uppercase tracking-widest mb-4">Configuration Error</h2>
                <p className="text-slate-400 font-bold mb-8 uppercase text-xs tracking-tighter italic">
        ...
        <div className="fixed inset-0 z-50 bg-black flex flex-col overflow-hidden">
            ...
            <div className="absolute top-4 left-4 right-4 flex justify-between items-center z-100 pointer-events-none">
                ...
                <div className="pointer-events-auto">
                    <button 
                        onClick={() => navigate(`/games/${game.slug}/session`)}
                        className="bg-black/40 backdrop-blur-md p-3 rounded-full text-white/50 hover:text-white transition-all border border-white/10 shadow-lg hover:scale-110 active:scale-95 group"
                        title="Exit Game"
                    >
