    ...
    <>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">
              Spend your PZA, spin to earn, represent Playza
            </p>
          </div>
        </div>

        {/* ── SPIN CARD ── */}
        <div className="relative overflow-hidden bg-linear-to-br from-indigo-600 via-purple-600 to-pink-600 rounded-2xl p-6 shadow-xl shadow-indigo-500/30">
          {/* BG decoration */}
          ...
          <div className="relative flex items-center justify-between gap-4">
            <div className="flex-1">
              ...
              <p className="text-white/70 text-sm mb-4">
                Win up to{" "}
                <span className="text-yellow-300 font-black">1,000 PZA</span>{" "}
                per spin. {spinsLeft} free spins today!
              </p>
              <button
            ...
            {/* Wheel preview icon */}
            <div
              className="shrink-0 w-24 h-24 rounded-full bg-white/10 border-2 border-white/20 flex items-center justify-center text-5xl shadow-inner cursor-pointer hover:scale-105 transition-transform"
              onClick={() => setShowSpinModal(true)}
            >
              🎰
            </div>
          ...
          <div className="relative mt-4 flex items-center gap-2">
            {Array.from({ length: 3 }).map((_, i) => (
              <div
                key={i}
                className={`h-1.5 flex-1 rounded-full ${i < spinsLeft ? "bg-yellow-300" : "bg-white/20"}`}
              />
            ))}
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-slate-600 dark:text-slate-400 mb-3 font-medium">
                Represent Playza, grow the community, earn exclusive perks and
                double your PZA rewards.
              </p>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-1.5 bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-200 dark:border-indigo-800 rounded-lg px-2.5 py-1">
              ...
              <span className="text-xs font-black text-indigo-600 dark:text-indigo-400">
                {totalPoints.toLocaleString()} PZA
              </span>
            </div>
          </div>
            ...
            {MERCH_PRODUCTS.map((product) => {
              ...
              return (
                ...
                >
                  {product.badge && (
                    <span
                      className={`absolute top-3 right-3 text-[9px] font-black text-white px-1.5 py-0.5 rounded-full ${product.badgeColor}`}
                    >
                      {product.badge}
                    </span>
                  )}
                  ...
                  {/* Product image */}
                  <div className="w-full h-24 bg-linear-to-br from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-750 rounded-xl flex items-center justify-center text-5xl group-hover:scale-105 transition-transform">
                    {product.image}
                  </div>
      ...
      {showAmbassadorModal && (
        ...
        >
          ...
          >
            <div className="w-14 h-14 rounded-2xl bg-linear-to-br from-amber-400 to-orange-500 flex items-center justify-center mx-auto mb-4 shadow-lg shadow-orange-400/30">
              <Crown className="w-7 h-7 text-white" />
            </div>
            <h3 className="font-black text-slate-900 dark:text-white text-xl text-center mb-1">
              Ambassador Program
            </h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 text-center mb-5">
              As a Playza Ambassador you'll get exclusive perks, double PZA, and
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-3 rounded-xl bg-linear-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-white font-black text-sm transition-all shadow-md shadow-orange-400/30 active:scale-95"
              >
                Apply Now 🚀
              </button>
            </div>
      ...
      {redeemModal && (
        ...
        >
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <div className="w-16 h-16 bg-linear-to-br from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-750 rounded-2xl flex items-center justify-center mx-auto mb-4 text-4xl">
              {redeemModal.image}
            </div>
            <h3 className="font-black text-slate-900 dark:text-white text-xl text-center mb-1">
              {redeemModal.name}
            ...
            <p className="text-center font-black text-indigo-600 dark:text-indigo-400 text-lg mb-5">
              {redeemModal.cost.toLocaleString()}{" "}
              <span className="text-sm text-slate-400 font-bold">PZA</span>
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setRedeemModal(null)}
              ...
              <button
                onClick={() => {
                  alert(
                    `Redemption request for "${redeemModal.name}" submitted! We'll contact you shortly.`,
                  );
                  setRedeemModal(null);
                }}
