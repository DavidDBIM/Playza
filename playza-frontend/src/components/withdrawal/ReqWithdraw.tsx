...
const ReqWithdraw = ({ onClick, status, amount, bank }: ReqWithdrawProps) => {
  ...
  return (
    <div className="glass-card rounded-xl py-8 px-2 md:p-12 relative overflow-hidden border border-yellow-500/20 shadow-2xl">
          ...
          <p className="text-slate-500 font-medium mb-12 relative z-10">Confirm destination and process your payout.</p>
          ...
          <div className="space-y-8 relative z-10">
            <div className="bg-slate-50 dark:bg-white/5 rounded-2xl px-2 py-6 md:p-6 border border-slate-200 dark:border-white/5">
                <h3 className="text-xs font-black uppercase tracking-widest text-slate-500 mb-4">Payout Details</h3>
                ...
                <div className="flex items-center justify-between">
                  <span className="font-black text-slate-900 dark:text-white uppercase tracking-widest text-sm">Payout Requested</span>
                  <span className="text-2xl font-black bg-clip-text text-transparent bg-linear-to-r from-yellow-400 via-amber-200 to-yellow-600">₦{amount.toLocaleString()}</span>
                </div>
              </div>

              <div className="bg-yellow-500/5 border border-yellow-500/20 rounded-xl px-2 py-4 md:p-4 flex gap-3 text-yellow-500/80">
              ...
              {status === "failed" && (
                <div className="bg-red-500/10 border border-red-500/20 rounded-xl px-2 py-4 md:p-4 flex gap-3 text-red-500 animate-in shake">
                  <XCircle className="shrink-0" />
                  <div>
                    <p className="font-bold">Execution Failed</p>
                    <p className="text-xs opacity-80 mt-1">Our gateway rejected the request. Please verify your limit and try again.</p>
                  </div>
                </div>
              )}
              ...
              <button
                ...
                className="w-full bg-linear-to-r from-yellow-500 to-amber-600 hover:from-yellow-400 hover:to-amber-500 text-white dark:text-background-dark py-4 rounded-xl font-black uppercase tracking-widest transition-all shadow-xl shadow-yellow-500/20 flex items-center justify-center gap-2 group hover:-translate-y-0.5 active:scale-[0.98]"
              >
                {status === "failed" ? (
