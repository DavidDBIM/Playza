...
const OtpPin = ({ onBack, onConfirm, amount, bank, isProcessing }: OtpPinProps) => {
  ...
  return (
    <div className="glass-card rounded-4xl md:rounded-[2.5rem] overflow-hidden shadow-2xl border border-slate-200 dark:border-white/10 relative">
      {/* Header Accent */}
      ...
      <div className="px-2 py-5 md:p-10 pt-16 md:pt-20">
        {/* Header */}
        <div className="flex flex-col items-center text-center mb-8 md:mb-10">
          <div className="size-16 md:size-20 rounded-3xl bg-yellow-500/10 border border-yellow-500/20 flex items-center justify-center shadow-inner mb-4 relative">
        ...
        <div className="space-y-8">
          {/* Summary Card */}
          <div className="bg-slate-50 dark:bg-white/5 rounded-2xl px-2 py-5 md:p-5 border border-slate-200 dark:border-white/10 space-y-4">
              ...
              <div className="flex justify-between gap-1.5 md:gap-2 max-w-sm mx-auto">
                {[...Array(6)].map((_, i) => (
                  <input
                    ...
                    maxLength={1}
                    inputMode="numeric"
                    className="w-full h-12 md:h-14 bg-slate-50 dark:bg-white/5 border-2 border-slate-200 dark:border-white/5 rounded-lg md:rounded-xl text-center text-xl font-black text-slate-900 dark:text-white focus:border-yellow-500/50 focus:bg-slate-100 dark:focus:bg-white/[0.07] outline-none transition-all"
                  />
                ))}
              </div>
              ...
              <div className="flex justify-center gap-3 md:gap-4">
                {[...Array(4)].map((_, i) => (
                  <input
                    ...
                    maxLength={1}
                    inputMode="numeric"
                    className="w-12 h-14 bg-slate-50 dark:bg-white/5 border-2 border-slate-200 dark:border-white/5 rounded-lg md:rounded-xl text-center text-xl font-black text-slate-900 dark:text-white focus:border-yellow-500/50 focus:bg-slate-100 dark:focus:bg-white/[0.07] outline-none transition-all"
                  />
                ))}
              </div>
            </div>
