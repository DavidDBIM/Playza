    ...
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="glass-card rounded-2xl p-6 md:p-8 relative overflow-hidden flex flex-col md:flex-row items-center gap-8 justify-between">
        ...
        <div className="flex flex-col z-10 flex-1 w-full relative">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full max-w-xl">
            <div className="flex-1 bg-slate-50 dark:bg-background/50 border border-slate-200 dark:border-white/10 rounded-xl flex items-center px-4 py-3 shadow-inner">
              <span className="text-slate-500 dark:text-slate-400 text-sm truncate mr-2 flex-1">{referralLink}</span>
            </div>
            <button 
              onClick={handleCopy}
        ...
        <div className="hidden md:flex flex-col items-center justify-center bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 p-6 rounded-2xl z-10 shrink-0">
          ...
          <p className="text-slate-900 dark:text-slate-300 font-bold text-sm">Scan to invite</p>
          <p className="text-slate-500 text-xs text-center">Perfect for local LAN games</p>
        </div>
      </div>
        ...
        {[
          ...
          <div key={i} className={`glass-card rounded-2xl p-4 md:p-6 flex flex-col justify-between items-start ${stat.glow || 'border-white/5'}`}>
            ...
            <div className="flex items-end gap-2">
              <span className={`text-2xl md:text-3xl font-black ${stat.label === 'TOTAL EARNED' ? stat.glow : 'text-slate-900 dark:text-slate-100'}`}>
                {stat.value}
              </span>
              {stat.trend && (
      ...
      <div className="glass-card rounded-2xl p-6 border-primary/20 bg-primary/5 relative overflow-hidden mt-2">
        <div className="flex justify-between items-center mb-4 relative z-10">
          ...
          <div className="text-right">
            <p className="text-slate-500 dark:text-slate-400 text-xs mb-1">Rewards Value</p>
            <p className="text-slate-900 dark:text-slate-100 font-black">$50.00</p>
          </div>
        </div>
