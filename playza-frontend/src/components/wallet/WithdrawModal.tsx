...
const WithdrawModal = ({ onClose }: WithdrawModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-md px-2 py-4 md:p-4 animate-in fade-in duration-300">
      <div className="relative w-full max-w-md glass-card rounded-3xl shadow-2xl border border-yellow-500/20 overflow-hidden">
        <div className="absolute -bottom-20 -right-20 w-48 h-48 bg-yellow-500/20 blur-[100px] rounded-full pointer-events-none"></div>
        ...
        <div className="px-2 py-6 md:p-8 relative z-10">
          <div className="flex items-center gap-4 mb-6 px-2">
            <div className="size-12 rounded-xl bg-yellow-500/10 border border-yellow-500/20 flex items-center justify-center shadow-inner shrink-0">
              <Landmark className="text-yellow-500 size-6" />
            </div>
            <div>
              <h2 className="text-xl md:text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tight">
                Quick <span className="bg-clip-text text-transparent bg-linear-to-r from-yellow-400 via-amber-200 to-yellow-600">Withdraw</span>
              </h2>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="space-y-3">
              <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 flex justify-between">
                <span>Amount (₦)</span>
                <span className="text-yellow-600 dark:text-yellow-500">Bal: ₦10,250</span>
              </label>
              
              <div className="relative group">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-yellow-500 font-black text-xl md:text-2xl opacity-50 group-focus-within:opacity-100 transition-opacity">
                  ₦
                </span>
                <input
                  type="text"
            ...
            <div className="space-y-3">
              ...
              <div className="grid gap-3">
                <button 
                  onClick={() => setBank("gtb")}
                  className={`flex flex-col px-2 py-3 md:p-3 rounded-xl border-2 transition-all text-left relative ${
                    bank === "gtb" 
                ...
                >
                  ...
                  {bank === "gtb" && <CheckCircle2 className="absolute right-3 top-1/2 -translate-y-1/2 text-yellow-500 size-5" />}
                </button>
                <button 
                ...
                >
                  ...
                  <span className="text-xs text-slate-500 font-medium">Guseltony</span>
                  {bank === "zenith" && <CheckCircle2 className="absolute right-3 top-1/2 -translate-y-1/2 text-yellow-500 size-5" />}
                </button>
              </div>
            </div>
              ...
              className="w-full bg-linear-to-r from-yellow-500 to-amber-600 hover:from-yellow-400 hover:to-amber-500 text-white dark:text-background-dark py-4 rounded-xl font-black uppercase tracking-widest shadow-lg shadow-yellow-500/20 hover:shadow-yellow-500/40 active:scale-[0.98] transition-all flex items-center justify-center gap-3 disabled:opacity-50 disabled:pointer-events-none group relative overflow-hidden text-sm"
            >
