  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/95 backdrop-blur-2xl px-2 py-2 md:py-4 md:p-4">
      <div className="relative w-full max-w-md glass-card rounded-xl border border-primary/20 overflow-hidden">
        ...
        <button 
          onClick={onClose}
          className="absolute top-4 right-4 p-2 rounded-xl bg-slate-100 md:hover:bg-slate-200 dark:bg-slate-800 md:dark:hover:bg-slate-700 text-slate-500 md:hover:text-slate-900 dark:text-slate-400 md:dark:hover:text-white z-20"
        >
          <X size={18} />
        </button>
          ...
          <div className="space-y-6">
            <div className="space-y-3">
              ...
              <div className="relative group">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 opacity-50 md:group-focus-within:opacity-100 scale-90">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-white/5 border-2 border-slate-200 dark:border-white/5 rounded-xl py-2 md:py-4 pl-12 pr-2 md:pr-4 text-xl md:text-2xl font-black text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-700 outline-none focus:border-primary/50"
                  placeholder="0"
                />
              </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {QUICK_AMOUNTS.map((val) => (
                  <button
                    ...
                    className={`relative py-2.5 rounded-lg text-xs font-black border overflow-hidden ${
                      Number(amount) === val
                      ? "bg-primary text-background dark:text-background-dark border-primary"
                      : "bg-slate-100 dark:bg-white/5 text-slate-500 border-slate-200 dark:border-white/5 md:hover:border-primary/30"
                    }`}
                  >
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2 md:gap-3">
                <button 
                  ...
                  className={`flex items-center gap-3 p-3 rounded-xl border-2 group text-left relative ${
                    paymentMethod === "card" 
                    ? "border-primary/50 bg-primary/5" 
                    : "border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-white/5 md:hover:border-slate-300 md:dark:hover:border-white/10"
                  }`}
                >
                ...
                <button 
                  ...
                  className={`flex items-center gap-2 md:gap-3 px-2 py-3 md:p-3 rounded-xl border-2 group text-left relative ${
            ...
            <button 
              ...
              className="w-full bg-primary text-background dark:text-background-dark py-2 md:py-4 rounded-xl font-black uppercase tracking-widest md:hover:bg-primary/90 flex items-center justify-center gap-2 md:gap-3 disabled:opacity-50 disabled:pointer-events-none group relative overflow-hidden text-sm"
            >
              <span>Continue</span>
