...
const Overview = () => {
  ...
  const wins = recentMatches.filter(
    (m: GameHistoryItem) => m.status === "completed" && m.winnings > 0,
  ).length;
  const winRate = totalGames > 0 ? Math.round((wins / totalGames) * 100) : 0;
  const highestScore = recentMatches.reduce(
    (max: number, m: GameHistoryItem) => Math.max(max, m.score || 0),
    0,
  );
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-2 md:gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700 overflow-hidden">
      <h2 className="md:hidden text-lg font-black text-slate-900 dark:text-white tracking-tight col-span-full">
        Overview
      </h2>
      ...
      <div className="xl:col-span-2 space-y-8">
        <div className="grid grid-cols-2 2xl:grid-cols-4 gap-2 md:gap-4 overflow-hidden">
          {[
            ...
            {
              label: "Highest Score",
              value: highestScore > 0 ? highestScore.toLocaleString() : "—",
              icon: <MdGrade />,
              sub: "Personal best",
            },
          ].map((stat, i) => (
            <div
              key={i}
              className="glass-card p-2 md:p-4 rounded-xl flex flex-col gap-1 hover:border-primary/40 transition-all group"
            >
              ...
              {recentMatches.map((match: GameHistoryItem, i: number) => (
                ...
                >
                  <div className="flex items-center gap-2 md:gap-4">
                    <div
                      className={`size-10 rounded-xl flex items-center justify-center text-xl ${match.winnings > 0 ? "bg-green-500/10" : "bg-red-500/10"}`}
                    >
                      {match.winnings > 0 ? (
                  ...
                  <div className="text-right">
                    <div
                      className={`flex items-center gap-1 justify-end font-black text-base ${match.winnings > 0 ? "text-primary" : "text-slate-500"}`}
                    >
                      {match.winnings > 0 ? (
                        <>
                          <span>+</span>
                          <ZASymbol className="text-sm scale-90" />
                          <span>{match.winnings.toLocaleString()}</span>
                        </>
                      ) : (
                        "—"
                      )}
