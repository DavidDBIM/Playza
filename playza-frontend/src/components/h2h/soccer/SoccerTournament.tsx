...
function makeBotTeam(idx: number, difficulty: Difficulty): TournamentTeam {
  return {
    ...
    colorIdx: (idx + 1) % TEAM_COLORS.length,
    isBot: true,
    difficulty,
  };
}
...
function advanceBracket(
  ...
  const updated = {
    ...
    matches: tournament.matches.map((m) =>
      m.id === match.id ? { ...m, status: "done" as const, winnerId } : m,
    ),
  };
  const nextRoundMatch = updated.matches.find(
    (m) =>
      m.round === match.round + 1 &&
      m.matchNum === Math.floor(match.matchNum / 2),
  );
  if (nextRoundMatch) {
  ...
  const currentRoundDone = updated.matches
...
// ─── Component ────────────────────────────────────────────────────────────────
const SoccerTournament: React.FC<SoccerTournamentProps> = ({
  userId,
  username,
}) => {
  const [phase, setPhase] = useState<
    "create" | "setup" | "bracket" | "match" | "champion"
  >("create");
  ...
  const handlePlayMatch = (match: Match) => {
    const playerInMatch =
      match.team0?.id === userId || match.team1?.id === userId;
  ...
  const handleMatchResult = (score: [number, number], winner: 0 | 1 | null) => {
    ...
    const updated = advanceBracket(
      tournament,
      updatedMatch,
      winnerTeam?.id || activeMatch.team0?.id || "",
    );
  ...
  if (phase === "bracket" && tournament) {
    ...
    const pendingMyMatch = tournament.matches.find(
      (m) =>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400 text-[10px] uppercase tracking-widest">
              {tournament.size} Teams · Round {tournament.currentRound}
            </p>
          </div>
          ...
          <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-2xl p-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-400 text-[10px] mt-1">
                vs{" "}
                {pendingMyMatch.team0?.id === userId
                  ? pendingMyMatch.team1?.name
                  : pendingMyMatch.team0?.name}
              </p>
            </div>
            <button
              ...
              return (
                <div key={r} className="flex flex-col gap-3">
                  ...
                  <div
                    ...
                    style={{
                      marginTop: rIdx === 0 ? 0 : `${Math.pow(2, rIdx) * 20}px`,
                    }}
                  >
                    {roundMatches.map((match) => {
                      ...
                      const myTeamInMatch =
                        match.team0?.id === userId ||
                        match.team1?.id === userId;
                      return (
                        <div
                          key={match.id}
                          className={`w-44 rounded-xl border transition-all ${match.status === "done" ? "border-white/5 bg-gray-900" : isPlaying ? "border-yellow-500/30 bg-yellow-500/5" : "border-white/5 bg-gray-900/50"}`}
                        >
                          {[match.team0, match.team1].map((team, ti) => (
                            <div
                              key={ti}
                              className={`flex items-center justify-between px-3 py-2 ${ti === 0 ? "border-b border-white/5" : ""} ${match.winnerId === team?.id ? "bg-green-500/10" : ""}`}
                            >
                              <div className="flex items-center gap-2 min-w-0">
                                {team ? (
                                  <div
                                    ...
                                    style={{
                                      backgroundColor:
                                        TEAM_COLORS[team.colorIdx]?.primary ||
                                        "#888",
                                    }}
                                  />
                                ) : (
                              ...
                              {match.status === "done" && (
                                <span
                                  className={`font-black text-xs tabular-nums ${match.winnerId === team?.id ? "text-green-400" : "text-gray-500"}`}
                                >
  ...
  if (phase === "match" && activeMatch && tournament) {
    const oppTeamObj =
      activeMatch.team0?.id === userId ? activeMatch.team1 : activeMatch.team0;
    ...
    return (
      <div className="w-full flex flex-col items-center pb-6">
        ...
        <SoccerGame
          ...
          team0Color={
            TEAM_COLORS[activeMatch.team0?.colorIdx || 0]?.primary || "#e63946"
          }
          team1Color={
            TEAM_COLORS[activeMatch.team1?.colorIdx || 1]?.primary || "#2196F3"
          }
          isBot={true}
    ...
    return (
      <div className="w-full min-h-[60vh] flex flex-col items-center justify-center space-y-6 py-12">
        ...
        <div className="text-center">
          ...
          <h2 className="text-white font-black text-3xl uppercase tracking-widest mt-1">
            {tournament.champion.name}
          </h2>
          {!tournament.champion.isBot && (
            <p className="text-yellow-400 font-black text-sm mt-2">
              Congratulations!
            </p>
          )}
        </div>
        <div
          className="w-20 h-20 rounded-full border-4 border-yellow-500 flex items-center justify-center"
          style={{
            backgroundColor:
              TEAM_COLORS[tournament.champion.colorIdx]?.primary || "#888",
          }}
        >
          {tournament.champion.isBot ? (
  ...
  return (
    <div className="w-full max-w-sm mx-auto px-4 py-8 space-y-6">
      ...
      <div className="space-y-4">
        <div>
          <label className="text-gray-400 text-[10px] uppercase tracking-widest font-black block mb-1">
            Tournament Name
          </label>
          <input
            value={tName}
        ...
        <div>
          <label className="text-gray-400 text-[10px] uppercase tracking-widest font-black block mb-2">
            Teams
          </label>
