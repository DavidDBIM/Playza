...
const TeamSetup: React.FC<TeamSetupProps> = ({ onConfirm, label }) => {
  ...
  return (
    <div className="bg-gray-900 border border-white/10 rounded-2xl p-6 space-y-5 max-w-sm mx-auto">
      <h3 className="text-white font-black text-lg uppercase tracking-widest text-center">
        {label}
      </h3>
      <div>
      ...
      <div>
        ...
        <div className="grid grid-cols-4 gap-2">
          {TEAM_COLORS.map((tc, i) => (
            <button
              ...
              onClick={() => {
                setColorIdx(i);
                if (!name || name === TEAM_COLORS[colorIdx].name)
                  setName(tc.name);
              }}
        ...
        <p className="text-yellow-400 font-black text-[10px] uppercase tracking-widest mt-2 text-center">
          {TEAM_COLORS[colorIdx].name}
        </p>
      </div>
      <button
...
const ModeSelect: React.FC<ModeSelectProps> = ({ onSelect }) => {
  const modes: {
  ...
  }[] = [
    {
      ...
      icon: <Timer size={20} />,
      label: "5 Min Match",
      desc: "Most goals in 5 minutes wins",
    },
    {
      mode: "sudden-death",
      icon: <Zap size={20} />,
      label: "Sudden Death",
      desc: "First goal wins instantly",
    },
    {
      mode: "tournament",
      icon: <Trophy size={20} />,
      label: "Tournament",
      desc: "Create a bracket with real players & AI",
    },
  ];
  return (
    <div className="space-y-3">
      <h3 className="text-white font-black text-center uppercase tracking-widest text-lg">
        Select Mode
      </h3>
...
// ─── Main Arena ───────────────────────────────────────────────────────────────
const SoccerArena: React.FC<SoccerArenaProps> = ({
  room: initialRoom,
  user,
}) => {
  ...
  const [gamePhase, setGamePhase] = useState<
    "mode-select" | "team-setup" | "playing" | "finished" | "tournament"
  >(initialRoom.is_bot ? "team-setup" : "mode-select");
  const [gameMode, setGameMode] = useState<GameMode>(
    initialRoom.game_mode || "timed",
  );
  ...
  const handleGoal = useCallback(() => {
    // Live score managed by game component
  }, []);

  const handleGameOver = useCallback(
    async (_: [number, number], winner: 0 | 1 | null) => {
      const winnerId =
        winner === null
          ? null
          : winner === myTeam
  ...
  const team0Color =
    TEAM_COLORS[isHost ? myColorIdx : hostColorIdx]?.primary || "#e63946";
  const team1Color =
    TEAM_COLORS[isHost ? guestColorIdx : myColorIdx]?.primary || "#2196F3";
  const team0Name = isHost
    ? myTeamName
    : room.team0_name || room.host?.username || "Team A";
  const team1Name = isHost
    ? room.team1_name || room.guest?.username || "Team B"
    : myTeamName;
  ...
  if (gamePhase === "team-setup") {
    return (
      <div className="w-full min-h-[60vh] flex items-center justify-center py-8">
        <div className="w-full max-w-sm px-4">
          <TeamSetup label="Your Team" onConfirm={handleTeamConfirm} />
        </div>
      </div>
    );
  }
    ...
    return (
      <H2HWinner
        room={{
          id: room.id,
          stake: room.stake,
          winner_id: room.winner_id || localWinnerId,
