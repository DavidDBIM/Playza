...
interface Ball {
  ...
  vx: number; vy: number;
  radius: number;
  drawPath: Vec2[];      // Score-Hero curved draw path
  isDrawing: boolean;
}
...
interface GameState {
  ...
  controlledIdx: number;   // index in players array
  isMyTeam0: boolean;
  myTeam: 0 | 1;
  kickoffTeam: 0 | 1;
  drawStart: Vec2 | null;
...
// ─── Constants ────────────────────────────────────────────────────────────────
const W = 800, H = 520;
const PITCH_MARGIN_X = 60, PITCH_MARGIN_Y = 40;
const PITCH_W = W - PITCH_MARGIN_X * 2;
const PITCH_H = H - PITCH_MARGIN_Y * 2;
const GOAL_W = 80, GOAL_H = 110;
const PLAYER_R = 14;
const BALL_R = 9;
...
const PITCH_THEMES: Record<PitchTheme, { grass1: string; grass2: string; line: string; soil: string; board: string }> = {
  classic: { grass1: "#3a8c3f", grass2: "#2d7a32", line: "rgba(255,255,255,0.9)", soil: "#8B6914", board: "#1a1a2e" },
  night:   { grass1: "#1a3a1f", grass2: "#142d18", line: "rgba(150,220,255,0.9)", soil: "#3a2a0a", board: "#0f0f1a" },
  snow:    { grass1: "#e8f4ff", grass2: "#d0e8f8", line: "rgba(100,150,220,0.9)", soil: "#c8d8e8", board: "#2a3a5a" },
  desert:  { grass1: "#c8a84b", grass2: "#b8942e", line: "rgba(255,255,255,0.8)", soil: "#8B6914", board: "#3a1a0a" },
  neon:    { grass1: "#0a1a0a", grass2: "#071207", line: "rgba(0,255,150,0.9)", soil: "#0a0a1a", board: "#000010" },
};
...
function makePlayers(isMyTeam0: boolean): Player[] {
  const players: Player[] = [];
  // Team 0 (left side) — keeper + 4 outfield
  const t0x = [PITCH_MARGIN_X + 25, PITCH_MARGIN_X + 150, PITCH_MARGIN_X + 200, PITCH_MARGIN_X + 180, PITCH_MARGIN_X + 210];
  const t0y = [H/2, H/2 - 90, H/2 + 90, H/2 - 30, H/2 + 30];
  for (let i = 0; i < 5; i++) {
    players.push({ id: `t0-${i}`, x: t0x[i], y: t0y[i], vx: 0, vy: 0, team: 0, isKeeper: i === 0, number: i + 1, radius: PLAYER_R, hasBall: false, name: `P${i+1}` });
  }
  // Team 1 (right side)
  const t1x = [W - PITCH_MARGIN_X - 25, W - PITCH_MARGIN_X - 150, W - PITCH_MARGIN_X - 200, W - PITCH_MARGIN_X - 180, W - PITCH_MARGIN_X - 210];
  const t1y = [H/2, H/2 - 90, H/2 + 90, H/2 - 30, H/2 + 30];
  for (let i = 0; i < 5; i++) {
    players.push({ id: `t1-${i}`, x: t1x[i], y: t1y[i], vx: 0, vy: 0, team: 1, isKeeper: i === 0, number: i + 1, radius: PLAYER_R, hasBall: false, name: `P${i+1}` });
  }
  return players;
}

function resetPositions(state: GameState, kickoffTeam: 0 | 1) {
  const players = makePlayers(state.isMyTeam0);
  const ball: Ball = { x: W / 2, y: H / 2, vx: 0, vy: 0, radius: BALL_R, drawPath: [], isDrawing: false };
  return { ...state, players, ball, phase: "kickoff" as const, kickoffTeam, lastGoalTeam: null, drawStart: null, drawEnd: null, controlledIdx: 0 };
}

// ─── AI Logic ─────────────────────────────────────────────────────────────────
function runAI(state: GameState, team: 1 | 0, dt: number): void {
  ...
  const speed = aiDifficulty === "easy" ? 1.4 : aiDifficulty === "medium" ? 2.2 : 3.0;
  const reactionJitter = aiDifficulty === "easy" ? 0.4 : aiDifficulty === "medium" ? 0.15 : 0.02;
  ...
  teamPlayers.forEach((p, idx) => {
    ...
    if (p.isKeeper) {
      ...
      ty = clamp(ball.y, PITCH_MARGIN_Y + GOAL_H / 2, H - PITCH_MARGIN_Y - GOAL_H / 2);
    } else {
      const ballOwnerTeam = players.find(q => q.hasBall)?.team;
      if (ballOwnerTeam === team) {
        // Attacking: closest to ball charges, others spread
        const ballDist = players.filter(q => q.team === team && !q.isKeeper)
          .map(q => dist(q, ball));
        ...
        if (isChaseBall) {
          ...
          tx = clamp(ball.x + (oppGoalX - ball.x) * 0.4, PITCH_MARGIN_X, W - PITCH_MARGIN_X);
          ty = spreadY;
        }
      } else {
        // Defending: track back toward own goal
        tx = clamp(ball.x * 0.5 + ownGoalX * 0.5, PITCH_MARGIN_X, W - PITCH_MARGIN_X);
    ...
    if (!p.isKeeper && dist(p, ball) < PLAYER_R + BALL_R + 4) {
      const power = aiDifficulty === "easy" ? 6 : aiDifficulty === "medium" ? 9 : 12;
      const dir = norm({ x: oppGoalX - ball.x, y: (H / 2 - ball.y) * 0.3 });
      ball.vx = dir.x * power + (Math.random() - 0.5) * (aiDifficulty === "easy" ? 3 : 1);
      ball.vy = dir.y * power + (Math.random() - 0.5) * (aiDifficulty === "easy" ? 3 : 1);
    }
  });
}

// ─── Physics step ────────────────────────────────────────────────────────────
function physicsTick(state: GameState, inputDir: Vec2, drawShot: Vec2 | null): GameState {
  const s = { ...state, players: state.players.map(p => ({ ...p })), ball: { ...state.ball } };
  ...
  if (drawShot && s.phase === "playing") {
    const nearest = players.filter(p => p.team === s.myTeam)
  ...
  if (ball.y - BALL_R < PITCH_MARGIN_Y) { ball.y = PITCH_MARGIN_Y + BALL_R; ball.vy *= -0.7; }
  if (ball.y + BALL_R > H - PITCH_MARGIN_Y) { ball.y = H - PITCH_MARGIN_Y - BALL_R; ball.vy *= -0.7; }

  // Goal check
  const goalTop = H / 2 - GOAL_H / 2;
  const goalBot = H / 2 + GOAL_H / 2;
  let newScore = [...s.score] as [number, number];
  let goalScored: 0 | 1 | null = null;

  // Left goal (team 1 scores)
  if (ball.x - BALL_R < PITCH_MARGIN_X && ball.y > goalTop && ball.y < goalBot) {
    newScore[1]++;
    goalScored = 1;
  }
  // Right goal (team 0 scores)
  if (ball.x + BALL_R > W - PITCH_MARGIN_X && ball.y > goalTop && ball.y < goalBot) {
    newScore[0]++;
    goalScored = 0;
  }
  // Side walls
  if (ball.x - BALL_R < PITCH_MARGIN_X && goalScored === null) { ball.x = PITCH_MARGIN_X + BALL_R; ball.vx *= -0.7; }
  if (ball.x + BALL_R > W - PITCH_MARGIN_X && goalScored === null) { ball.x = W - PITCH_MARGIN_X - BALL_R; ball.vx *= -0.7; }
  ...
  if (s.phase === "playing") {
    const myPlayers = players.map((p, i) => ({ p, i })).filter(({ p }) => p.team === s.myTeam);
    const nearest = myPlayers.sort((a, b) => dist(a.p, ball) - dist(b.p, ball))[0];
    if (nearest) s.controlledIdx = nearest.i;
  }

  // Sudden death check
  if (goalScored !== null && s.gameMode === "sudden-death") {
    return { ...s, score: newScore, phase: "finished", lastGoalTeam: goalScored };
  }
...
function render(
  ...
  for (let i = 0; i < 8; i++) {
    ...
    ctx.fillRect(PITCH_MARGIN_X + (PITCH_W / 8) * i, PITCH_MARGIN_Y, PITCH_W / 8, PITCH_H);
  }
  ...
  const boardTexts = ["PLAYZA", "⚽ SCORE HERO", "PLAYZA.GG", "JOIN NOW", "⚡ PLAYZA", "WIN BIG", "PLAYZA", "H2H ZONE"];
  ctx.font = "bold 10px Arial";
  ctx.textAlign = "center";
  const segW = PITCH_W / boardTexts.length;
  boardTexts.forEach((txt, i) => {
    ctx.fillStyle = i % 2 === 0 ? "#FFD700" : "#ffffff";
    ctx.fillText(txt, PITCH_MARGIN_X + segW * i + segW / 2, PITCH_MARGIN_Y - boardH + 10);
    ctx.fillText(txt, PITCH_MARGIN_X + segW * i + segW / 2, H - PITCH_MARGIN_Y + 18);
  });
  ...
  if (drawPathPoints.length > 1) {
    ...
    ctx.fillStyle = "rgba(255,220,0,0.9)"; ctx.fill();
  }

  // ── Players
  state.players.forEach(p => {
    const col = p.team === 0 ? team0Color : team1Color;
    const isControlled = state.players.indexOf(p) === state.controlledIdx && p.team === state.myTeam;

    // Shadow
    ctx.beginPath(); ctx.ellipse(p.x, p.y + p.radius - 2, p.radius * 0.8, 4, 0, 0, Math.PI * 2);
    ...
    if (isControlled) {
      ctx.beginPath(); ctx.arc(p.x, p.y - p.radius - 6, 4, 0, Math.PI * 2);
  ...
  // ── Ball
  const b = state.ball;
  ctx.beginPath(); ctx.ellipse(b.x, b.y + b.radius - 2, b.radius * 0.8, 3, 0, 0, Math.PI * 2);
...
const SoccerGame: React.FC<SoccerGameProps> = ({
  ...
  const [overlayPhase, setOverlayPhase] = useState<"kickoff" | "goal" | "finished" | null>("kickoff");
  ...
  const initState = useCallback((): GameState => {
    const s: GameState = {
      players: makePlayers(myTeam === 0),
      ball: { x: W/2, y: H/2, vx: 0, vy: 0, radius: BALL_R, drawPath: [], isDrawing: false },
      score: [0, 0],
    ...
    const loop = (ts: number) => {
      ...
      if (s.phase === "kickoff") {
        ...
        while (accumulatedTime >= 16) {
          accumulatedTime -= 16;
          stateRef.current = physicsTick(stateRef.current!, inputRef.current, drawShotRef.current);
      ...
      render(ctx, stateRef.current!, theme, team0Color, team1Color, team0Name, team1Name, drawPathRef.current);
  ...
  }, [initState, team0Color, team1Color, team0Name, team1Name, gameMode, theme]);

  // ── Keyboard controls
  useEffect(() => {
    const down = new Set<string>();
    const onKey = (e: KeyboardEvent, pressed: boolean) => {
      if (pressed) down.add(e.key); else down.delete(e.key);
      const dir = { x: 0, y: 0 };
  ...
  const onPointerUp = (e: React.TouchEvent | React.MouseEvent) => {
    ...
    setTimeout(() => { drawPathRef.current = []; }, 300);
    ...
    <div className="flex flex-col items-center w-full select-none">
      ...
      <div className="flex items-center justify-between w-full max-w-3xl px-2 py-2 bg-gray-900 rounded-t-xl border-b border-white/10">
        <div className="flex items-center gap-2">
          <div className="w-5 h-5 rounded-full border-2 border-white/30" style={team0BgStyle} />
          <span className="text-white font-black text-xs uppercase tracking-wider truncate max-w-20">{team0Name}</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-white font-black text-2xl tabular-nums">{score[0]}</span>
          <div className="flex flex-col items-center">
            <span className="text-yellow-400 font-black text-xs">{gameMode === "timed" ? formatTime(timeLeft) : "SUDDEN DEATH"}</span>
            <span className="text-gray-500 text-[9px] uppercase tracking-widest">Score Hero</span>
          </div>
          <span className="text-white font-black text-2xl tabular-nums">{score[1]}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-white font-black text-xs uppercase tracking-wider truncate max-w-20">{team1Name}</span>
          <div className="w-5 h-5 rounded-full border-2 border-white/30" style={team1BgStyle} />
        </div>
      </div>
        ...
        {overlayPhase === "kickoff" && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div className="text-center">
              <div className="text-yellow-400 font-black text-3xl uppercase tracking-widest">⚽ Kick Off!</div>
              <div className="text-white/70 text-sm mt-2 uppercase tracking-widest">Draw a path to shoot</div>
            </div>
          </div>
        )}
        ...
        {stateRef.current?.phase === "goal" && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="text-center animate-bounce">
              <div className="text-yellow-400 font-black text-5xl uppercase tracking-widest drop-shadow-2xl">⚽ GOAL!</div>
        ...
        {overlayPhase === "finished" && stateRef.current && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/70 backdrop-blur-md rounded-none">
            <div className="text-center space-y-4">
              <div className="text-yellow-400 font-black text-4xl uppercase">Full Time!</div>
              <div className="flex items-center gap-6 justify-center">
                <div className="text-center">
                  <div className="text-4xl font-black text-white">{score[0]}</div>
                  <div className="text-xs text-gray-400 uppercase tracking-widest">{team0Name}</div>
                </div>
                <div className="text-2xl text-gray-500">–</div>
                <div className="text-center">
                  <div className="text-4xl font-black text-white">{score[1]}</div>
                  <div className="text-xs text-gray-400 uppercase tracking-widest">{team1Name}</div>
                </div>
              </div>
              <div className="text-white font-black text-xl uppercase tracking-widest">
                {score[0] > score[1] ? `🏆 ${team0Name} Wins!` : score[1] > score[0] ? `🏆 ${team1Name} Wins!` : "🤝 Draw!"}
              </div>
            </div>
          </div>
        )}
        ...
        {showPitchPicker && (
          <div className="absolute top-8 right-2 bg-gray-900/95 border border-white/20 rounded-xl p-3 z-50 space-y-1">
            {(Object.keys(PITCH_THEMES) as PitchTheme[]).map(t => (
              <button
                key={t}
                onClick={() => { setTheme(t); setShowPitchPicker(false); }}
      ...
      <div className="flex justify-between w-full max-w-3xl px-3 py-2 bg-gray-900 rounded-b-xl">
        <span className="text-gray-500 text-[9px] uppercase tracking-widest">← Drag left side to move</span>
        <span className="text-gray-500 text-[9px] uppercase tracking-widest">Drag right side to shoot →</span>
      </div>
    </div>
  );
};
...
export { TEAM_COLORS, PITCH_THEMES, type PitchTheme, type Difficulty, type GameMode };
