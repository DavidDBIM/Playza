...
const H2HWinner = ({ room, user }: H2HWinnerProps) => {
      ...
      <div className="relative z-10 w-full max-w-3xl text-center space-y-12">
        ...
        <div className="relative flex flex-col items-center">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
                <h1 className={`font-headline text-8xl font-black italic tracking-tighter uppercase leading-none drop-shadow-3xl rotate-[-5deg] animate-in slide-in-from-bottom-5 duration-700 ${isWinner ? 'text-white' : 'text-muted-foreground'}`}>
        ...
        {/* Reward Stats */}
        <div className="glass backdrop-blur-3xl p-10 rounded-[3rem] border border-white/20 shadow-3xl text-center relative overflow-hidden group">
            ...
            <div className={`font-headline text-8xl font-black italic tracking-tighter leading-none mb-8 drop-shadow-2xl ${isWinner ? 'text-secondary' : 'text-foreground'}`}>
              {isWinner ? `+${winningAmount.toFixed(0)}` : `-${room.stake}`} <span className="text-2xl not-italic font-sans opacity-50 uppercase tracking-widest">ZA</span>
            </div>
            
            <div className="h-px bg-white/10 w-48 mx-auto mb-10"></div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2 p-4 rounded-3xl bg-white/5 border border-white/5 hover:border-primary/30 transition-all">
                    <p className="text-[9px] font-black uppercase text-muted-foreground tracking-widest italic">Rival</p>
                    <p className="font-headline font-black text-2xl text-foreground italic">
                        {isWinner ? (room.guest?.id === user?.id ? room.host?.username : room.guest?.username) : "Tough opponent"}
                    </p>
                </div>
                <div className="space-y-2 p-4 rounded-3xl bg-white/5 border border-white/5 hover:border-primary/30 transition-all">
                    <p className="text-[9px] font-black uppercase text-muted-foreground tracking-widest italic">Experience</p>
                    <p className="font-headline font-black text-2xl text-foreground italic">
        ...
        <div className="flex flex-col sm:flex-row gap-6 justify-center">
          <NavLink 
            to="/h2h"
            className="px-12 py-6 bg-white dark:bg-black/60 border-[3px] border-primary text-primary font-headline font-black rounded-3xl transition-all hover:scale-110 shadow-2xl uppercase tracking-widest text-lg italic hover:bg-primary hover:text-white"
          >
            NEXT BATTLE
          </NavLink>
          <NavLink 
            to="/leaderboard"
            className="px-12 py-6 bg-foreground text-background font-headline font-black rounded-3xl transition-all hover:scale-110 shadow-2xl uppercase tracking-widest text-lg italic hover:shadow-primary/30"
          >
            LEADERBOARDS
          </NavLink>
        </div>
        ...
        <div className="pt-8 opacity-50 hover:opacity-100 transition-opacity">
           <button className="text-[9px] font-black uppercase tracking-[0.6em] text-muted-foreground italic flex items-center justify-center gap-4 hover:text-primary transition-colors">
