...
const QuickMatchView = ({
  ...
  return (
    <div className="w-full max-w-xl mx-auto animate-in slide-in-from-right-8 duration-500">
      <button
        onClick={() => setView("hub")}
        className="flex items-center gap-2 text-[10px] sm:text-xs font-black uppercase tracking-widest text-slate-500 hover:text-white mb-6 md:mb-8 transition-colors"
      >
        ← Back to Hub
      </button>
              ...
              <div className="space-y-3 max-h-80 overflow-y-auto no-scrollbar pr-1">
                {publicRooms.map((room) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between p-2 rounded-xl bg-white/5 border border-white/10 hover:border-indigo-500/50 hover:bg-indigo-500/5 transition-all group"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl overflow-hidden border border-white/10">
                        <img src={room.host?.avatar_url || `https://api.dicebear.com/7.x/bottts/svg?seed=${room.host?.username}`} alt={room.host?.username} className="w-full h-full object-cover" />
                      </div>
