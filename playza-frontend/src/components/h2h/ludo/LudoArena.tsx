  ...
  return (
    <div className="flex flex-col gap-1 w-full max-w-full mx-auto p-1 box-border md:p-6 min-h-screen bg-slate-50 dark:bg-slate-950 overflow-y-auto">
      ...
      <div className="flex items-center justify-between bg-white dark:bg-slate-900 border border-slate-300 dark:border-white/10 rounded-xl px-1 py-1.5 md:px-5 md:py-3 shadow-sm relative z-20">
        <button onClick={toggleFullscreen} className="absolute -top-10 right-2 p-2 bg-slate-200 dark:bg-slate-800 rounded-lg text-slate-600 hidden md:flex items-center justify-center shadow-md hover:bg-slate-300">
        ...
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          <div className="min-w-0 overflow-hidden">
            <span className="font-black text-[10px] md:text-sm text-slate-900 dark:text-white uppercase truncate">{room.host?.username || "Host"}</span>
            <div className="text-[9px] md:text-[10px] text-slate-500 font-bold uppercase truncate border-b-2 border-red-500 inline-block">Red & Yellow ({isHost ? "YOU" : "RIVAL"})</div>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center shrink-0 px-2 flex-none">
          ...
          <button onClick={toggleFullscreen} className="p-1.5 md:hidden bg-slate-100 rounded-md text-slate-500 mb-1 active:scale-95">
          ...
          <div className="flex items-center gap-0.5 bg-amber-500/10 border border-amber-500/20 rounded-md px-1.5 py-0.5 text-amber-500 font-black text-[10px]">
             <Trophy size={9} /> {room.stake * 2} <ZASymbol className="w-2.5 h-2.5" />
          </div>
        </div>

        <div className="flex items-center justify-end gap-2 min-w-0 flex-1 text-right">
          <div className="min-w-0 overflow-hidden">
            <span className="font-black text-[10px] md:text-sm text-slate-900 dark:text-white uppercase truncate">{oppUsername}</span>
            <div className="text-[9px] md:text-[10px] text-slate-500 font-bold uppercase truncate border-b-2 border-emerald-400 inline-block">Green & Blue ({!isHost ? "YOU" : "RIVAL"})</div>
          </div>
          <div className="flex gap-1 relative">
      ...
      <div className="flex flex-col md:grid md:grid-cols-[1fr_auto_1fr] flex-1 gap-2 md:gap-6 items-start mt-2 md:mt-4">
        ...
        <div className="flex md:flex-col items-stretch gap-2 w-full md:w-64">
           ...
           <div className="flex-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 p-3 md:p-6 rounded-2xl flex flex-col justify-center shadow-lg relative overflow-hidden">
             <h3 className="uppercase font-black text-slate-400 text-[10px] md:text-xs tracking-widest mb-1 md:mb-3">Current Turn</h3>
             <div className="flex items-center gap-2">
               <div className={`w-3 h-3 md:w-4 md:h-4 rounded-full ${turnBadgeColor} animate-pulse`} />
               <div className={`text-sm md:text-2xl font-black uppercase ${turnTextColor}`}>{turnName}</div>
             </div>
             {isMyTurn && <div className="absolute right-0 bottom-0 bg-indigo-500 text-white font-black uppercase text-[9px] md:text-xs px-2 md:px-4 py-1 rounded-tl-xl animate-pulse">Your Turn</div>}
           </div>

           <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 p-4 md:p-8 rounded-2xl flex flex-col items-center justify-center shadow-lg cursor-pointer hover:bg-slate-50 transition-colors" 
                onClick={executeRoll}
                style={{ opacity: !isMyTurn || gameState.hasRolled || gameState.winner ? 0.6 : 1, pointerEvents: !isMyTurn || gameState.hasRolled || gameState.winner ? 'none' : 'auto' }}
            >
             <h3 className="uppercase font-black text-slate-400 text-[9px] md:text-xs tracking-widest mb-2 md:mb-6">Tap to Roll</h3>
             <motion.div animate={isRolling ? { rotate: [0, 90, 180, 270, 360], scale: [1, 1.1, 1] } : {}} transition={{ duration: 0.3, repeat: isRolling ? Infinity : 0 }}>
                <DiceFace value={isRolling ? animatedDice : (gameState.diceValue || 1)} />
             </motion.div>
             {gameState.hasRolled && isMyTurn && !gameState.winner && !isRolling && (
               <div className="mt-2 md:mt-6 text-[9px] md:text-xs font-black text-indigo-500 uppercase">Move a piece</div>
             )}
           </div>

           <button onClick={handleResign} className="hidden md:block w-full px-4 py-4 bg-red-500/10 hover:bg-red-500/20 text-red-500 rounded-xl font-black uppercase transition-colors text-sm shadow-md">
              Resign Game
           </button>
        </div>
        ...
        <div className="flex flex-col gap-2 w-full md:w-64 h-48 md:h-[500px]">
          <div className="flex-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-2xl overflow-hidden flex flex-col shadow-lg">
            ...
            <div className="flex-1 overflow-y-auto p-2 flex flex-col gap-1 md:gap-2">
              {gameState.history.map((log, i) => (
                <div key={i} className="text-[9px] md:text-xs text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/30 p-2 rounded-lg font-mono border border-slate-100 dark:border-slate-800/50 shadow-sm leading-tight">
                  {log}
                </div>
              ))}
            </div>
          ...
          <button onClick={handleResign} className="md:hidden w-full px-4 py-3 bg-red-500/10 hover:bg-red-500/20 text-red-500 rounded-xl font-black uppercase transition-colors text-xs text-center shadow-inner">
              Resign Game
          </button>
        </div>
      </div>
