  ...
  return (
    <div className="p-4 space-y-6 max-w-5xl">
      ...
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-2xl bg-linear-to-br from-violet-400 to-purple-600 flex items-center justify-center shadow-md shadow-violet-400/30">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-card border border-border rounded-2xl shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-border bg-linear-to-r from-violet-500/5 to-purple-500/5">
          ...
          <div className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  },
                  {
                    value: "za_token",
                    label: "ZA Token",
                    icon: <MdToken className="text-base" />,
                    color: "amber",
                  },
                ].map((opt) => (
                  <button
                    key={opt.value}
                    onClick={() => {
                      setRewardType(opt.value as "pza" | "za_token");
                      setAmount("");
                    }}
            ...
            <div className="relative">
              ...
              <div className="relative">
                ...
                />
                {searching && (
                  <MdRefresh className="-translate-y-1/2 absolute right-3 top-1/2 text-muted-foreground animate-spin" />
                )}
              </div>
            ...
            {selectedUser && (
              <div className="flex items-center gap-3 px-3 py-2.5 bg-violet-50 dark:bg-violet-950/30 border border-violet-200 dark:border-violet-800 rounded-xl">
                <div className="w-7 h-7 rounded-full bg-linear-to-br from-violet-400 to-purple-500 flex items-center justify-center text-white text-xs font-black shrink-0">
            ...
            {selectedUser && amount && (
              ...
              >
                ...
                <span className="font-black">
                  ...
                  {rewardType === "pza" ? "PZA Points" : "ZA Tokens"}
                </span>{" "}
                to <span className="font-black">@{selectedUser.username}</span>
              </div>
            )}
            ...
            <button
              ...
              className={`w-full py-3 rounded-xl font-black text-sm transition-all flex items-center justify-center gap-2 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed active:scale-95 ${
                rewardType === "pza"
                  ? "bg-linear-to-r from-violet-500 to-purple-600 hover:from-violet-400 hover:to-purple-500 text-white shadow-violet-500/30"
                  : "bg-linear-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-white shadow-amber-500/30"
              }`}
            >
              {granting ? (
                <>
        ...
        <div className="bg-card border border-border rounded-2xl shadow-sm overflow-hidden flex flex-col">
          <div className="px-5 py-4 border-b border-border flex items-center justify-between bg-linear-to-r from-slate-500/5 to-slate-500/0">
            <h2 className="font-black text-foreground text-base flex items-center gap-2">
              <MdHistory className="text-lg text-muted-foreground" /> Grant
              History
            </h2>
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                <ChevronDown className="-translate-y-1/2 absolute right-2 top-1/2 w-3 h-3 text-muted-foreground pointer-events-none" />
              </div>
              <button
              ...
              >
                <MdRefresh
                  className={`text-base ${histLoading ? "animate-spin" : ""}`}
                />
              </button>
            </div>
          </div>
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2.5">
                      <div className="w-8 h-8 rounded-full bg-linear-to-br from-slate-200 to-slate-300 dark:from-slate-700 dark:to-slate-800 flex items-center justify-center text-foreground text-xs font-black shrink-0">
                        {log.target_user?.username?.[0]?.toUpperCase() ?? (
                          <MdPerson />
                        )}
                      </div>
                      <div>
                    ...
                    <div className="text-right shrink-0">
                      <p
                        className={`text-sm font-black ${log.type === "pza" ? "text-violet-600 dark:text-violet-400" : "text-amber-600 dark:text-amber-400"}`}
                      >
          ...
          {(history?.pages ?? 0) > 1 && (
            <div className="px-4 py-3 border-t border-border flex items-center justify-between">
              <p className="text-xs text-muted-foreground">
                Page {histPage} of {history?.pages}
              </p>
              <div className="flex gap-2">
                ...
                <button
                  onClick={() =>
                    setHistPage((p) => Math.min(history?.pages ?? 1, p + 1))
                  }
                  disabled={histPage === (history?.pages ?? 1)}
                  className="px-3 py-1.5 rounded-lg text-xs font-bold bg-muted hover:bg-muted/80 disabled:opacity-40 transition-all"
                >
                  Next →
                </button>
