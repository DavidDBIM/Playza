...
const QUAL_COLORS: Record<string, string> = {
  social_influencer: "bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300",
  gold_badge: "bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300",
  referral_100: "bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300",
};
const STATUS_CONFIG = {
...
const Ambassadors: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const { data: res } = await apiClient.get("/admin/ambassadors", { params });
      setData(res.data);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  }, [page, search, statusFilter, qualFilter]);
  ...
  async function handleReview(action: "approve" | "reject") {
    ...
    try {
      await apiClient.patch(`/admin/ambassadors/${selectedApp.id}/review`, {
        action,
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-foreground tracking-tight">Ambassador Applications</h1>
            <p className="text-sm text-muted-foreground font-medium">Review and manage brand ambassador requests</p>
          </div>
        </div>
        <button onClick={fetchData} className="flex items-center gap-2 px-4 py-2 bg-muted hover:bg-muted/80 rounded-xl text-sm font-bold text-foreground transition-all">
          <MdRefresh className={`text-lg ${loading ? "animate-spin" : ""}`} /> Refresh
        </button>
      </div>
      ...
      {data && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            { label: "Pending",  value: data.applications.filter(a=>a.status==="pending").length,  color: "text-amber-600 dark:text-amber-400" },
            { label: "Approved", value: data.applications.filter(a=>a.status==="approved").length, color: "text-emerald-600 dark:text-emerald-400" },
      ...
      <div className="bg-card border border-border rounded-2xl p-4 flex flex-wrap gap-3 items-center shadow-sm">
        <form onSubmit={handleSearch} className="flex gap-2 flex-1 min-w-50">
          ...
          <button type="submit" className="px-4 py-2.5 bg-primary text-primary-foreground rounded-xl text-sm font-bold hover:bg-primary/90 transition-all">Search</button>
        </form>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <div className="relative">
            <select value={qualFilter} onChange={e => { setQualFilter(e.target.value); setPage(1); }}
              className="appearance-none pl-3 pr-8 py-2.5 rounded-xl bg-muted border border-border text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary cursor-pointer transition-all">
      ...
      <div className="bg-card border border-border rounded-2xl shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-muted-foreground font-bold">No applications found</p>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-muted/50">
                    {["Applicant", "Qualification", "Followers", "Platforms", "Status", "Applied", "Actions"].map(h => (
                    ...
                    return (
                      <tr key={app.id} className="hover:bg-muted/30 transition-colors group">
                        <td className="px-4 py-3.5">
                          <div>
                            <p className="font-bold text-foreground text-sm">{app.full_name}</p>
                            <p className="text-xs text-muted-foreground">{app.email}</p>
                            {app.users?.username && (
                              <p className="text-[10px] text-primary font-bold mt-0.5">@{app.users.username}</p>
                            )}
                          </div>
                        </td>
                        <td className="px-4 py-3.5">
                          <span className={`text-[10px] font-black px-2 py-1 rounded-lg ${QUAL_COLORS[app.qualification_type]}`}>
                        ...
                        <td className="px-4 py-3.5">
                          <button
                            onClick={() => { setSelectedApp(app); setReviewNote(app.admin_note ?? ""); setReviewError(""); }}
          ...
          <div className="bg-card border border-border rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-border sticky top-0 bg-card z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-black text-foreground text-base">{selectedApp.full_name}</h3>
                  <p className="text-xs text-muted-foreground">{selectedApp.email}</p>
                </div>
              </div>
              <button onClick={() => setSelectedApp(null)} className="w-8 h-8 rounded-xl bg-muted hover:bg-muted/80 flex items-center justify-center text-muted-foreground hover:text-foreground transition-all">
            ...
            <div className="px-5 py-5 space-y-4">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <span className={`inline-flex items-center gap-1 text-[10px] font-black px-2.5 py-1 rounded-lg ${STATUS_CONFIG[selectedApp.status].cls}`}>
                  {STATUS_CONFIG[selectedApp.status].icon} {STATUS_CONFIG[selectedApp.status].label}
                </span>
                <span className={`text-[10px] font-black px-2.5 py-1 rounded-lg ${QUAL_COLORS[selectedApp.qualification_type]}`}>
                ...
                {selectedApp.users?.username && (
                  <span className="text-[10px] font-black px-2.5 py-1 rounded-lg bg-primary/10 text-primary flex items-center gap-1">
                    <MdVerified className="text-sm" /> @{selectedApp.users.username}
                  </span>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {selectedApp.phone && (
                  <div className="bg-muted/50 rounded-xl p-3">
                    ...
                    <p className="text-sm font-bold text-foreground">{selectedApp.phone}</p>
                  </div>
                )}
                {selectedApp.follower_count && (
                  <div className="bg-muted/50 rounded-xl p-3">
                    <p className="text-[10px] font-black text-muted-foreground uppercase tracking-wider mb-1">Followers</p>
                    <p className="text-sm font-bold text-foreground">{selectedApp.follower_count.toLocaleString()}</p>
                  </div>
                )}
                {selectedApp.content_niche && (
                  <div className="bg-muted/50 rounded-xl p-3">
                    <p className="text-[10px] font-black text-muted-foreground uppercase tracking-wider mb-1">Content Niche</p>
                    <p className="text-sm font-bold text-foreground">{selectedApp.content_niche}</p>
                  </div>
                )}
                <div className="bg-muted/50 rounded-xl p-3">
                  <p className="text-[10px] font-black text-muted-foreground uppercase tracking-wider mb-1">Applied</p>
                  <p className="text-sm font-bold text-foreground">{new Date(selectedApp.created_at).toLocaleDateString()}</p>
                </div>
              </div>
              ...
              {selectedApp.platforms && selectedApp.platforms.length > 0 && (
                <div className="bg-muted/50 rounded-xl p-3">
                  <p className="text-[10px] font-black text-muted-foreground uppercase tracking-wider mb-2">Platforms & Handles</p>
                  <div className="space-y-1.5">
                    {selectedApp.platforms.map(p => (
                      <div key={p} className="flex items-center justify-between">
                        <span className="text-xs font-black text-foreground uppercase">{p}</span>
              ...
              {/* Motivation */}
              <div className="bg-muted/50 rounded-xl p-3">
                <p className="text-[10px] font-black text-muted-foreground uppercase tracking-wider mb-2">Motivation</p>
                <p className="text-sm text-foreground leading-relaxed">{selectedApp.motivation}</p>
              </div>
              ...
              {selectedApp.status === "pending" ? (
                ...
                <div className={`py-3 rounded-xl text-center text-sm font-black ${STATUS_CONFIG[selectedApp.status].cls}`}>
                  This application has been <span className="uppercase">{selectedApp.status}</span>
                  {selectedApp.reviewed_at && <span className="font-normal text-xs block mt-0.5">on {new Date(selectedApp.reviewed_at).toLocaleDateString()}</span>}
                </div>
              )}
            </div>
          </div>
