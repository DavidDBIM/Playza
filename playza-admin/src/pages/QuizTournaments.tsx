...
function QuestionManagerModal({ tournament, onClose }: { tournament: QuizTournament; onClose: () => void }) {
  ...
  const [newQ, setNewQ] = useState({ question_text: "", option_a: "", option_b: "", option_c: "", option_d: "", correct_option: "A" });
  ...
  const { mutate: addQ, isPending: adding } = useMutation({
    ...
    onSuccess: () => { refetch(); queryClient.invalidateQueries({ queryKey: ["admin-quiz-tournaments"] }); setNewQ({ question_text: "", option_a: "", option_b: "", option_c: "", option_d: "", correct_option: "A" }); setAddMode(false); },
  });
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.85)", backdropFilter: "blur(16px)" }} onClick={() => setConfirmCancel(null)}>
          <div className="w-full max-w-sm rounded-2xl overflow-hidden" style={{ background: "linear-gradient(135deg, rgba(25,8,8,0.99), rgba(10,3,3,0.99))", border: "1px solid rgba(239,68,68,0.2)", boxShadow: "0 0 60px rgba(239,68,68,0.1)" }} onClick={e => e.stopPropagation()}>
            <div className="p-6 text-center">
              ...
              <p className="text-xs text-white/30 mb-6">This will cancel the tournament. Registered players will be notified. This cannot be undone.</p>
              <div className="flex gap-3">
                <button onClick={() => setConfirmCancel(null)} className="flex-1 py-2.5 rounded-xl font-bold text-sm text-white/50 hover:text-white transition-all" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }}>Keep It</button>
