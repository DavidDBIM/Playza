...
interface QuizTournament {
  ...
  max_players: number | null; prize_distribution: PrizeTier[] | null;
...
interface QuizQuestion {
  id: string; round_number: number; question_text: string;
  option_a: string; option_b: string; option_c: string; option_d: string;
  correct_option: string; difficulty: string; time_limit_secs: number;
  order_index: number; image_url?: string | null;
}
...
const api = {
  ...
  getSponsors:      async (): Promise<Sponsor[]> => { const { data } = await apiClient.get("/admin/quiz/sponsors"); return data.data ?? []; },
  attachSponsor:    async (tournamentId: string, payload: { sponsor_id?: string | null; sponsor_mode?: string | null; sponsor_banner_url?: string | null }) => {
    const { data } = await apiClient.patch(`/admin/quiz/sponsors/attach/${tournamentId}`, payload);
    return data.data;
  },
};
...
function PrizeDistributionBuilder({ tiers, onChange, platformFeePct }: { tiers: PrizeTier[]; onChange: (t: PrizeTier[]) => void; platformFeePct: number; }) {
  ...
  const distributablePreview = Math.round(1000 * (1 - platformFeePct / 100));
  function addTier() { const nextRank = tiers.length > 0 ? Math.max(...tiers.map(t => t.rank)) + 1 : 1; onChange([...tiers, { rank: nextRank, percentage: 0 }]); }
  function removeTier(rank: number) { onChange(tiers.filter(t => t.rank !== rank)); }
  function updatePct(rank: number, pct: number) { onChange(tiers.map(t => t.rank === rank ? { ...t, percentage: Math.max(0, Math.min(maxAllocatable, pct)) } : t)); }
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <label className={labelCls} style={{ margin: 0 }}>Prize Distribution<span style={{ fontWeight: 400, textTransform: "none", opacity: 0.5, marginLeft: 4 }}>({maxAllocatable}% distributable after {platformFeePct}% fee)</span></label>
        <span className="text-[10px] font-black" style={{ color: totalPct > maxAllocatable ? "#f87171" : totalPct === maxAllocatable ? "#4ade80" : "#fbbf24" }}>{totalPct}% / {maxAllocatable}% allocated</span>
      </div>
      <div className="h-2 rounded-full overflow-hidden mb-3" style={{ background: "rgba(255,255,255,0.06)" }}>
        <div className="h-full rounded-full transition-all" style={{ width: `${Math.min(totalPct / maxAllocatable * 100, 100)}%`, background: totalPct > maxAllocatable ? "#ef4444" : "linear-gradient(90deg, #7c3aed, #a855f7)" }} />
      </div>
      <div className="space-y-2 mb-3">
        {tiers.length === 0 && <p className="text-center text-xs text-white/25 py-3">No tiers yet — click Add Rank below</p>}
        {tiers.map((tier) => (
          <div key={tier.rank} className="flex items-center gap-2 rounded-xl px-3 py-2" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
            ...
            <div className="relative flex-1">
              <input type="number" min={0} max={maxAllocatable} value={tier.percentage} onChange={e => updatePct(tier.rank, Number(e.target.value))} className={inputCls + " pr-8"} style={{ paddingTop: 6, paddingBottom: 6 }} placeholder="0" />
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-black text-white/30">%</span>
            </div>
            <button onClick={() => removeTier(tier.rank)} className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0 transition-all" style={{ background: "rgba(239,68,68,0.12)", color: "#f87171" }}><MdClose className="text-sm" /></button>
          </div>
        ))}
      </div>
      <div className="flex items-center gap-2">
        <button onClick={addTier} type="button" className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold text-white/60 hover:text-white transition-all" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)" }}><MdAdd /> Add Rank</button>
        {tiers.length > 0 && remaining > 0 && <span className="text-[10px] text-white/25">{remaining}% unallocated</span>}
        {totalPct > maxAllocatable && <span className="text-[10px] font-black text-red-400">⚠ Exceeds {maxAllocatable}% limit</span>}
      </div>
      {isValid && totalPct > 0 && (
        <div className="mt-3 rounded-xl px-3 py-2 space-y-1" style={{ background: "rgba(168,85,247,0.06)", border: "1px solid rgba(168,85,247,0.15)" }}>
          <p className="text-[9px] font-black text-white/25 uppercase tracking-widest mb-1.5">Preview (based on 1,000 ZA pool · {distributablePreview} ZA after {platformFeePct}% fee)</p>
          {tiers.map(tier => (
            <div key={tier.rank} className="flex items-center justify-between">
              ...
              <span className="text-xs font-black text-white">{Math.round(distributablePreview * tier.percentage / 100).toLocaleString()} ZA</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="w-full max-w-lg max-h-[90vh] flex flex-col rounded-2xl overflow-hidden" style={{ background: "linear-gradient(135deg, rgba(30,20,60,0.98) 0%, rgba(15,10,40,0.98) 100%)", border: "1px solid rgba(139,92,246,0.3)", boxShadow: "0 0 60px rgba(139,92,246,0.2), inset 0 1px 0 rgba(255,255,255,0.05)" }} onClick={e => e.stopPropagation()}>
        <div className="px-6 py-5 border-b border-white/5 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: "linear-gradient(135deg, #7c3aed, #4f46e5)", boxShadow: "0 0 20px rgba(124,58,237,0.4)" }}><Trophy className="w-4 h-4 text-white" /></div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={f.key}><label className={labelCls}>{f.label}</label><input type={f.type} placeholder={f.placeholder} value={(form as any)[f.key]} onChange={e => setForm(p => ({ ...p, [f.key]: e.target.value }))} className={inputCls} /></div>
            ))}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.85)", backdropFilter: "blur(16px)" }} onClick={onClose}>
      <div className="w-full max-w-lg max-h-[90vh] flex flex-col rounded-2xl overflow-hidden" style={{ background: "linear-gradient(135deg, rgba(20,30,60,0.98) 0%, rgba(10,15,40,0.98) 100%)", border: "1px solid rgba(59,130,246,0.3)", boxShadow: "0 0 60px rgba(59,130,246,0.15), inset 0 1px 0 rgba(255,255,255,0.05)" }} onClick={e => e.stopPropagation()}>
        <div className="px-6 py-5 border-b border-white/5 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: "linear-gradient(135deg, #2563eb, #0ea5e9)", boxShadow: "0 0 20px rgba(37,99,235,0.4)" }}><MdEdit className="text-white text-lg" /></div>
            <div><h3 className="font-black text-white text-base tracking-tight">Edit Tournament</h3><p className="text-white/40 text-[10px] font-bold uppercase tracking-wider">{tournament.title}</p></div>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-xl flex items-center justify-center text-white/40 hover:text-white hover:bg-white/10 transition-all"><MdClose /></button>
        </div>
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              <MdHandshake className="text-violet-400" />
              <p className="text-[10px] font-black text-white/40 uppercase tracking-widest">Sponsor Branding <span className="font-normal normal-case opacity-50">(optional)</span></p>
            </div>
...
function QuestionManagerModal({ tournament, onClose }: { tournament: QuizTournament; onClose: () => void }) {
  ...
  const { mutate: deleteQ } = useMutation({ mutationFn: api.deleteQuestion, onSuccess: () => { refetch(); queryClient.invalidateQueries({ queryKey: ["admin-quiz-tournaments"] }); } });
  ...
  function handleFileImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = (ev) => { try { const parsed = JSON.parse(ev.target?.result as string); const qs = Array.isArray(parsed) ? parsed : parsed.questions; if (!Array.isArray(qs)) throw new Error("Expected an array of questions"); bulkImport(qs); } catch (err: any) { setImportError(err.message ?? "Invalid JSON"); } };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.85)", backdropFilter: "blur(16px)" }} onClick={onClose}>
      <div className="w-full max-w-3xl max-h-[90vh] flex flex-col rounded-2xl overflow-hidden" style={{ background: "linear-gradient(160deg, rgba(15,10,35,0.99) 0%, rgba(5,5,20,0.99) 100%)", border: "1px solid rgba(255,255,255,0.08)", boxShadow: "0 0 80px rgba(0,0,0,0.8)" }} onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 shrink-0">
          <div><h3 className="font-black text-white text-base">{tournament.title}</h3><p className="text-white/30 text-xs font-bold">{questions.length} questions total</p></div>
          <div className="flex items-center gap-2">
        ...
        {(importError || importSuccess) && <div className="mx-6 mt-4 px-4 py-2.5 rounded-xl text-sm font-bold border shrink-0" style={importSuccess ? { background: "rgba(16,185,129,0.1)", border: "1px solid rgba(16,185,129,0.3)", color: "#34d399" } : { background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.3)", color: "#f87171" }}>{importSuccess || importError}</div>}
        ...
        <div className="flex gap-2 px-6 py-3 border-b border-white/5 shrink-0 overflow-x-auto">
          {ROUND_META.map(r => { const count = questions.filter(q => q.round_number === r.round).length; const active = activeRound === r.round; return (
            <button key={r.round} onClick={() => { setActiveRound(r.round); setAddMode(false); }} className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-black whitespace-nowrap transition-all" style={active ? { background: r.bg, border: `1px solid ${r.border}`, color: r.color } : { background: "rgba(255,255,255,0.03)", border: "1px solid transparent", color: "rgba(255,255,255,0.3)" }}>
              <span style={active ? { color: r.color } : {}}>{r.icon}</span><span>{r.name}</span>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-2">
          {roundQuestions.length === 0 && !addMode ? (
            <div className="py-12 text-center"><MdQuestionMark className="text-4xl mx-auto mb-2 opacity-10 text-white" /><p className="font-bold text-sm text-white/30">No questions for {rm?.name}</p></div>
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    {(["A","B","C","D"] as const).map(opt => (
                      <div key={opt} className="flex items-center gap-1.5 text-xs px-2 py-1 rounded-lg" style={q.correct_option === opt ? { background: "rgba(34,197,94,0.1)", border: "1px solid rgba(34,197,94,0.3)", color: "#4ade80" } : { color: "rgba(255,255,255,0.3)" }}>
                        <span className="font-black text-[10px]">{opt}</span>{(q as any)[`option_${opt.toLowerCase()}`]}
                        {q.correct_option === opt && <MdCheckCircle className="ml-auto text-green-400 shrink-0" />}
                      </div>
                    ))}
                  </div>
                </div>
                <button onClick={() => deleteQ(q.id)} className="opacity-0 group-hover:opacity-100 w-7 h-7 rounded-lg flex items-center justify-center transition-all" style={{ background: "rgba(239,68,68,0.15)", color: "#f87171" }}><MdDelete className="text-sm" /></button>
              </div>
            </div>
          ))}
          {addMode && (
        ...
        {!addMode && <div className="px-6 py-4 border-t border-white/5 shrink-0"><button onClick={() => setAddMode(true)} className="w-full py-2.5 rounded-xl font-bold text-sm transition-all flex items-center justify-center gap-2" style={{ background: "rgba(255,255,255,0.03)", border: `1px dashed ${rm?.border}`, color: rm?.color }}><MdAdd /> Add Question to {rm?.name}</button></div>}
      </div>
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.85)", backdropFilter: "blur(16px)" }} onClick={onClose}>
      <div className="w-full max-w-2xl max-h-[85vh] flex flex-col rounded-2xl overflow-hidden" style={{ background: "linear-gradient(160deg, rgba(20,5,5,0.99) 0%, rgba(8,2,2,0.99) 100%)", border: "1px solid rgba(239,68,68,0.2)" }} onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          {isLoading ? (
            <div className="py-16 flex flex-col items-center justify-center gap-3 text-white/40"><MdRefresh className="animate-spin text-3xl" /><p className="text-sm font-bold">Loading...</p></div>
          ) : isError ? (
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                { label: isActive ? "Eliminated" : "Players", value: isActive ? (live?.stats?.eliminated ?? 0) : (tournament as any).player_count ?? 0, color: isActive ? "#ef4444" : "#22c55e" },
              ].map(s => (
            ...
            {isActive && (
              <div className="rounded-xl px-4 py-3 flex items-center justify-between" style={{ background: "rgba(34,197,94,0.06)", border: "1px solid rgba(34,197,94,0.15)" }}>
                <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-green-400 animate-pulse" /><span className="text-sm font-black text-green-400">Still Alive</span></div>
                <span className="text-2xl font-black text-green-400">{live?.stats?.alive ?? 0}</span>
              </div>
            )}
            <div className="rounded-2xl overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.06)" }}>
              <div className="px-4 py-3 border-b border-white/5 flex items-center justify-between">
                <p className="text-xs font-black text-white/30 uppercase tracking-wider">{isActive ? "Live Leaderboard" : "Registered Players"}</p>
                <span className="text-xs font-black text-white/20">{live?.leaderboard?.length ?? 0} players</span>
              </div>
              {!live?.leaderboard || live.leaderboard.length === 0 ? (
                <div className="py-12 text-center"><p className="text-sm font-black text-white/30">{isActive ? "No leaderboard data yet" : "No players registered yet"}</p></div>
              ) : (
                <div className="divide-y divide-white/5 max-h-72 overflow-y-auto">
                  {live.leaderboard.slice(0, 50).map((entry: any, i: number) => (
                    <div key={i} className="flex items-center gap-3 px-4 py-2.5">
                      <span className="text-sm font-black text-white/30 w-5 text-center shrink-0">{i === 0 ? "👑" : i === 1 ? "🥈" : i === 2 ? "🥉" : i + 1}</span>
                      <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-black text-white shrink-0" style={{ background: "rgba(124,58,237,0.3)" }}>{entry.username?.[0]?.toUpperCase() ?? "?"}</div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-white truncate">{entry.username ?? "Unknown"}</p>
                        {isActive && <p className="text-[10px] text-white/30">{entry.correct_answers ?? 0} correct</p>}
                      </div>
                      <span className="text-[10px] font-black px-2 py-0.5 rounded-full shrink-0" style={entry.status === "winner" ? { background: "rgba(234,179,8,0.15)", color: "#fbbf24" } : entry.status === "alive" ? { background: "rgba(34,197,94,0.15)", color: "#4ade80" } : entry.status === "eliminated" ? { background: "rgba(239,68,68,0.15)", color: "#f87171" } : { background: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.3)" }}>{entry.status ?? "registered"}</span>
                    </div>
                  ))}
                </div>
              )}
...
function TournamentCard({ t, onManageQ, onEdit, onMonitor, onStart, onLaunch, onDisable, onDelete, startPending, launchPending }: { t: QuizTournament; onManageQ: () => void; onEdit: () => void; onMonitor: () => void; onStart: () => void; onLaunch: () => void; onDisable: () => void; onDelete: () => void; startPending: boolean; launchPending: boolean; }) {
  const sc = STATUS_CFG[t.status] ?? STATUS_CFG.draft;
  const isLive = t.status === "active"; const isReg = t.status === "registration"; const isLobby = t.status === "lobby"; const isDraft = t.status === "draft"; const isDone = t.status === "completed" || t.status === "cancelled";
  return (
    <div className="relative rounded-2xl overflow-hidden flex flex-col transition-all duration-300 group" style={{ background: "linear-gradient(160deg, rgba(20,15,40,0.95) 0%, rgba(10,8,25,0.95) 100%)", border: `1px solid ${isLive ? "rgba(239,68,68,0.4)" : isReg ? "rgba(34,197,94,0.3)" : "rgba(255,255,255,0.07)"}`, boxShadow: isLive ? "0 0 40px rgba(239,68,68,0.12), 0 8px 32px rgba(0,0,0,0.4)" : "0 8px 32px rgba(0,0,0,0.3)" }}>
      ...
      <div className="px-5 pt-5 pb-4">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex-1 min-w-0"><h3 className="font-black text-white text-base leading-tight truncate">{t.title}</h3>{t.description && <p className="text-white/35 text-xs mt-1 line-clamp-1">{t.description}</p>}</div>
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full shrink-0" style={{ background: `${sc.color}18`, border: `1px solid ${sc.color}40` }}>
            <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: sc.color, ...(isLive ? { animation: "pulse 1.5s infinite" } : {}) }} />
            <span className="text-[10px] font-black uppercase tracking-wider" style={{ color: sc.color }}>{sc.label}</span>
          </div>
        </div>
        {t.scheduled_at && <div className="flex items-center gap-1.5 mb-3"><Calendar className="w-3 h-3 text-white/25" /><span className="text-[11px] font-bold text-white/30">{new Date(t.scheduled_at).toLocaleDateString("en-NG", { weekday: "short", day: "numeric", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" })}</span></div>}
      ...
      <div className="px-5 py-3 border-t border-white/5">
        <div className="flex items-center gap-1">
          {ROUND_META.map(r => { const done = t.status === "completed" || (isLive && t.current_round > r.round); const current = isLive && t.current_round === r.round; return (
            <div key={r.round} className="flex-1 flex flex-col items-center gap-1">
              <div className="w-full h-1 rounded-full" style={{ background: done || current ? r.color : "rgba(255,255,255,0.06)", boxShadow: current ? `0 0 8px ${r.color}` : "none" }} />
              <span className="text-[8px] font-bold hidden sm:block" style={{ color: current ? r.color : "rgba(255,255,255,0.2)" }}>{r.name.split(" ")[0]}</span>
            </div>
          ); })}
        </div>
        {t.entry_fee > 0 && <div className="flex items-center gap-1 mt-2"><MdBolt className="text-amber-400 text-xs" /><span className="text-[10px] font-bold text-white/30">{t.entry_fee} ZA entry fee</span></div>}
      </div>
      <div className="px-4 pb-4 pt-1 flex flex-wrap gap-2 mt-auto">
        <button onClick={onManageQ} className="flex items-center gap-1 px-3 py-2 rounded-xl text-xs font-bold text-white/60 hover:text-white transition-all" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }}><MdEdit className="text-sm" /> Questions</button>
        {!isDone && <button onClick={onEdit} className="flex items-center gap-1 px-3 py-2 rounded-xl text-xs font-bold transition-all" style={{ background: "rgba(59,130,246,0.1)", border: "1px solid rgba(59,130,246,0.2)", color: "#60a5fa" }}><MdEdit className="text-sm" /> Edit</button>}
        {(isLive || isReg || isLobby) && <button onClick={onMonitor} className="flex items-center gap-1 px-3 py-2 rounded-xl text-xs font-bold transition-all" style={{ background: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.3)", color: "#f87171" }}><Eye className="w-3 h-3" /> {isLive ? "Monitor Live" : "View Players"}</button>}
        {isDraft && <>
          {(t.question_count ?? 0) < 5 && <div className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-bold" style={{ background: "rgba(245,158,11,0.1)", border: "1px solid rgba(245,158,11,0.3)", color: "#fbbf24" }}><MdWarning className="text-sm shrink-0" /><span>{t.question_count ?? 0} / 5 questions — add more to open registration</span></div>}
          <button onClick={onStart} disabled={startPending} className="flex items-center gap-1 px-3 py-2 rounded-xl text-xs font-black text-white transition-all disabled:opacity-50" style={{ background: "linear-gradient(135deg, rgba(34,197,94,0.3), rgba(16,185,129,0.2))", border: "1px solid rgba(34,197,94,0.4)" }}>{startPending ? <MdRefresh className="animate-spin" /> : <MdPlayArrow />}{startPending ? "Opening..." : "Open Registration"}</button>
        </>}
        {(isReg || isLobby) && <button onClick={onLaunch} disabled={launchPending} className="flex items-center gap-1 px-3 py-2 rounded-xl text-xs font-black text-white transition-all disabled:opacity-40" style={{ background: "linear-gradient(135deg, #7c3aed, #4f46e5)", boxShadow: "0 0 20px rgba(124,58,237,0.3)" }}>{launchPending ? <MdRefresh className="animate-spin" /> : <Zap className="w-3 h-3" />}{launchPending ? "Launching..." : "Launch Game Now"}</button>}
        {isDone && <button onClick={onDelete} className="flex items-center gap-1 px-3 py-2 rounded-xl text-xs font-black transition-all ml-auto" style={{ background: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.3)", color: "#f87171" }}><MdDelete className="text-sm" /> Delete</button>}
        {!isDone && <button onClick={onDisable} className="flex items-center gap-1 px-3 py-2 rounded-xl text-xs font-bold transition-all ml-auto" style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", color: "#f87171" }}><MdClose className="text-sm" /> Disable</button>}
      </div>
    </div>
  );
}
...
function RevenueTab({ tournaments }: { tournaments: QuizTournament[] }) {
  const totalRevenue = tournaments.reduce((s, t) => { const f = (t as any).platform_fee_percentage ?? 10; return s + Math.round(t.prize_pool * f / 100); }, 0);
  const totalPool = tournaments.reduce((s, t) => s + t.prize_pool, 0);
  const totalPlayers = tournaments.reduce((s, t) => s + t.player_count, 0);
  const totalPaid = tournaments.reduce((s, t) => { const f = (t as any).platform_fee_percentage ?? 10; return s + Math.round(t.prize_pool * (1 - f / 100)); }, 0);
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[{ label: "Total Revenue", value: `${totalRevenue.toLocaleString()} ZA`, color: "#4ade80", icon: "💰" }, { label: "Total Entry Fees", value: `${totalPool.toLocaleString()} ZA`, color: "#a855f7", icon: "🎯" }, { label: "Prizes Paid Out", value: `${totalPaid.toLocaleString()} ZA`, color: "#f87171", icon: "🏆" }, { label: "Total Players", value: totalPlayers.toLocaleString(), color: "#fbbf24", icon: "👥" }].map(s => (
      ...
      <div className="rounded-2xl overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.07)" }}>
        ...
        {tournaments.length === 0 ? <div className="py-12 text-center"><p className="text-white/20 text-sm font-bold">No tournaments yet</p></div> : (
          <div className="divide-y divide-white/5">
            <div className="grid px-5 py-2" style={{ gridTemplateColumns: "1fr 60px 80px 60px 90px 80px" }}>
              {["Tournament","Players","Pool","Fee %","Revenue","Status"].map(h => <p key={h} className="text-[9px] font-black text-white/20 uppercase tracking-widest">{h}</p>)}
            </div>
            {tournaments.map(t => { const f = (t as any).platform_fee_percentage ?? 10; const rev = Math.round(t.prize_pool * f / 100); const sc = STATUS_CFG[t.status] ?? STATUS_CFG.draft; return (
              <div key={t.id} className="grid px-5 py-3 items-center hover:bg-white/[0.02]" style={{ gridTemplateColumns: "1fr 60px 80px 60px 90px 80px" }}>
                <div className="min-w-0 pr-3"><p className="text-sm font-bold text-white truncate">{t.title}</p>{t.scheduled_at && <p className="text-[10px] text-white/25 mt-0.5">{new Date(t.scheduled_at).toLocaleDateString("en-NG", { day: "numeric", month: "short", year: "numeric" })}</p>}</div>
                ...
                <p className="text-sm font-black" style={{ color: "#fbbf24" }}>{f}%</p>
                <p className="text-sm font-black" style={{ color: rev > 0 ? "#4ade80" : "rgba(255,255,255,0.3)" }}>{rev > 0 ? `+${rev.toLocaleString()} ZA` : "—"}</p>
                <span className="text-[10px] font-black px-2 py-1 rounded-full w-fit" style={{ background: `${sc.color}18`, color: sc.color }}>{t.status}</span>
              </div>
            ); })}
          </div>
        )}
...
function LeaderboardTab({ tournaments }: { tournaments: QuizTournament[] }) {
  ...
  const { data: globalLb = [], isLoading: globalLoading } = useQuery({ queryKey: ["admin-global-leaderboard"], queryFn: async () => { const { data } = await apiClient.get("/admin/quiz/leaderboard/global"); return data.data ?? []; }, retry: 1 });
  const { data: tournamentLb = [], isLoading: tLoading } = useQuery({ queryKey: ["admin-tournament-leaderboard", selectedT], queryFn: async () => { if (!selectedT) return []; const { data } = await apiClient.get(`/admin/quiz/tournaments/${selectedT}/leaderboard`); return data.data ?? []; }, enabled: !!selectedT && lbTab === "per-tournament", retry: 1 });
  const MEDALS: Record<number, string> = { 0: "👑", 1: "🥈", 2: "🥉" };
  return (
    <div className="space-y-4">
      <div className="flex gap-1 border-b border-white/10">
        {([{ id: "global", label: "🌍 All-Time Global" }, { id: "per-tournament", label: "🏆 Per Tournament" }] as const).map(tab => (
          <button key={tab.id} onClick={() => setLbTab(tab.id)} className="px-4 py-2.5 text-xs font-black uppercase tracking-widest transition-all -mb-px" style={{ background: "none", border: "none", cursor: "pointer", borderBottom: lbTab === tab.id ? "2px solid #a855f7" : "2px solid transparent", color: lbTab === tab.id ? "#a855f7" : "rgba(255,255,255,0.3)" }}>{tab.label}</button>
        ))}
      </div>
      {lbTab === "per-tournament" && (
        <div className="relative">
          <select value={selectedT} onChange={e => setSelectedT(e.target.value)} className="w-full px-4 py-2.5 rounded-xl text-sm font-bold text-white appearance-none focus:outline-none" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)" }}>
            {tournaments.length === 0 ? <option value="">No tournaments yet</option> : tournaments.map(t => <option key={t.id} value={t.id} style={{ background: "#1e1035" }}>{t.title} ({t.status})</option>)}
          </select>
      ...
      <div className="rounded-2xl overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.07)" }}>
        <div className="px-5 py-3 border-b border-white/5" style={{ background: "rgba(255,255,255,0.02)" }}>
          <p className="text-xs font-black text-white/40 uppercase tracking-widest">{lbTab === "global" ? "All-Time Top Players" : "Tournament Leaderboard"}</p>
        </div>
        {(lbTab === "global" ? globalLoading : tLoading) ? (
          <div className="py-12 flex items-center justify-center gap-2 text-white/30"><MdRefresh className="animate-spin text-xl" /><span className="text-sm font-bold">Loading...</span></div>
        ) : (lbTab === "global" ? globalLb : tournamentLb).length === 0 ? (
          <div className="py-12 text-center"><p className="text-sm font-black text-white/30">{lbTab === "global" ? "No global data yet" : "No leaderboard data"}</p></div>
        ) : (
          <div className="divide-y divide-white/5">
            <div className="grid px-5 py-2" style={{ gridTemplateColumns: "40px 1fr 80px 80px 80px" }}>
              {["#","Player","Correct", lbTab === "global" ? "Wins" : "Avg Time","Status"].map(h => <p key={h} className="text-[9px] font-black text-white/20 uppercase tracking-widest">{h}</p>)}
            </div>
            {(lbTab === "global" ? globalLb : tournamentLb).map((entry: any, i: number) => (
              <div key={i} className="grid px-5 py-3 items-center hover:bg-white/[0.02]" style={{ gridTemplateColumns: "40px 1fr 80px 80px 80px", background: i === 0 ? "rgba(168,85,247,0.04)" : "transparent" }}>
                <span className="text-base">{MEDALS[i] ?? <span className="text-xs font-black text-white/30">{i + 1}</span>}</span>
                <div className="flex items-center gap-2 min-w-0">
                  <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-black text-white shrink-0" style={{ background: i === 0 ? "rgba(168,85,247,0.4)" : "rgba(255,255,255,0.08)" }}>{entry.username?.[0]?.toUpperCase() ?? "?"}</div>
                  <p className="text-sm font-bold text-white truncate">{entry.username ?? "Unknown"}</p>
                </div>
                <p className="text-sm font-black" style={{ color: "#4ade80" }}>{entry.correct_answers ?? 0}</p>
                <p className="text-sm font-black text-white/50">{lbTab === "global" ? (entry.total_wins ?? 0) : entry.avg_time_ms ? `${(entry.avg_time_ms / 1000).toFixed(1)}s` : "—"}</p>
                <span className="text-[10px] font-black px-2 py-0.5 rounded-full w-fit" style={entry.status === "winner" ? { background: "rgba(234,179,8,0.15)", color: "#fbbf24" } : entry.status === "alive" ? { background: "rgba(34,197,94,0.15)", color: "#4ade80" } : entry.status === "eliminated" ? { background: "rgba(239,68,68,0.12)", color: "#f87171" } : { background: "rgba(255,255,255,0.05)", color: "rgba(255,255,255,0.3)" }}>{entry.status ?? "—"}</span>
              </div>
            ))}
          </div>
        )}
...
const QuizTournaments: React.FC = () => {
  ...
  const patchCache = (id: string, patch: Partial<QuizTournament>) => { queryClient.setQueryData<QuizTournament[]>(["admin-quiz-tournaments"], old => (old ?? []).map(t => t.id === id ? { ...t, ...patch } : t)); };
  const { mutate: startT } = useMutation({
    ...
    onSuccess: (_d, id) => { setPendingStart(null); patchCache(id, { status: "registration" }); showToast("Registration is now open!"); },
    onError: (err: any) => { setPendingStart(null); queryClient.invalidateQueries({ queryKey: ["admin-quiz-tournaments"] }); setStartError(err?.response?.data?.message ?? "Failed to open registration"); },
  });
  const { mutate: launchT } = useMutation({
    ...
    onSuccess: (_d, id) => { setPendingLaunch(null); patchCache(id, { status: "active" }); showToast("🚀 Game launched!"); },
    onError: (err: any) => { setPendingLaunch(null); queryClient.invalidateQueries({ queryKey: ["admin-quiz-tournaments"] }); showToast(err?.response?.data?.message ?? "Failed to launch", "err"); },
  });
  ...
  const { mutate: deleteT } = useMutation({
    ...
    onSuccess: (_d, id) => { queryClient.setQueryData<QuizTournament[]>(["admin-quiz-tournaments"], old => (old ?? []).filter(t => t.id !== id)); setConfirmDelete(null); showToast("Tournament deleted."); },
    onError: (err: any) => { showToast(err?.response?.status === 404 ? "Delete endpoint not found — redeploy backend." : err?.response?.data?.message ?? "Failed to delete", "err"); },
  });
  ...
  return (
    <div className="min-h-screen p-4 sm:p-6" style={{ background: "linear-gradient(160deg, #0a0618 0%, #050310 50%, #080515 100%)" }}>
      {toast && (
        <div className="fixed top-6 right-6 z-[100] px-5 py-3 rounded-2xl font-bold text-sm flex items-center gap-2" style={toast.type === "ok" ? { background: "rgba(16,185,129,0.15)", border: "1px solid rgba(16,185,129,0.35)", color: "#34d399" } : { background: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.35)", color: "#f87171" }}>
          {toast.type === "ok" ? <MdCheckCircle className="text-lg shrink-0" /> : <MdWarning className="text-lg shrink-0" />}{toast.msg}
        </div>
      )}
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-4">
            <div className="relative w-12 h-12 rounded-2xl flex items-center justify-center" style={{ background: "linear-gradient(135deg, #7c3aed 0%, #4f46e5 100%)", boxShadow: "0 0 30px rgba(124,58,237,0.5)" }}><Crown className="w-6 h-6 text-white" /></div>
            <div><h1 className="text-2xl font-black text-white tracking-tight">Quiz Tournaments</h1><p className="text-white/35 text-sm font-medium">Create, manage and monitor live quiz games</p></div>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => refetch()} className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-bold text-white/60 hover:text-white transition-all" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }}><MdRefresh className={`text-lg ${isLoading ? "animate-spin" : ""}`} /> Refresh</button>
            <button onClick={() => setCreateOpen(true)} className="flex items-center gap-1.5 px-5 py-2.5 rounded-xl text-sm font-black text-white hover:opacity-90" style={{ background: "linear-gradient(135deg, #7c3aed, #4f46e5)", boxShadow: "0 0 24px rgba(124,58,237,0.4)" }}><MdAdd className="text-lg" /> New Tournament</button>
          </div>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[{ label: "Total", value: stats.total, color: "#a855f7", icon: <Trophy className="w-4 h-4" /> }, { label: "Live Now", value: stats.live, color: "#ef4444", icon: <div className="w-2 h-2 rounded-full bg-red-500 animate-pulse" /> }, { label: "Registration", value: stats.reg, color: "#22c55e", icon: <MdPlayArrow className="text-base" /> }, { label: "Total Players", value: stats.players, color: "#f59e0b", icon: <Users className="w-4 h-4" /> }].map(s => (
            <div key={s.label} className="rounded-2xl px-4 py-3 flex items-center gap-3" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
              ...
              <div><p className="text-xl font-black text-white leading-none">{s.value}</p><p className="text-[10px] font-bold text-white/30 uppercase tracking-wider mt-0.5">{s.label}</p></div>
            </div>
          ))}
        </div>
        <div className="border-b border-white/10">
          <div className="flex gap-0">
            {([{ id: "tournaments", label: "🏟️ Tournaments" }, { id: "revenue", label: "💰 Revenue" }, { id: "leaderboard", label: "🏆 Leaderboard" }] as const).map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} className="px-5 py-3 text-xs font-black uppercase tracking-widest transition-all -mb-px" style={{ background: "none", border: "none", cursor: "pointer", borderBottom: activeTab === tab.id ? "2px solid #7c3aed" : "2px solid transparent", color: activeTab === tab.id ? "#a855f7" : "rgba(255,255,255,0.3)" }}>{tab.label}</button>
            ))}
          </div>
        </div>
        {activeTab === "revenue" ? <RevenueTab tournaments={tournaments} /> : activeTab === "leaderboard" ? <LeaderboardTab tournaments={tournaments} /> : isLoading ? (
      ...
      {editT && <EditTournamentModal tournament={editT} onClose={() => setEditT(null)} onSaved={(updated) => { setEditT(updated); queryClient.setQueryData<QuizTournament[]>(["admin-quiz-tournaments"], old => (old ?? []).map(t => t.id === updated.id ? { ...t, ...updated } : t)); showToast("Tournament updated!"); }} />}
      ...
      {needsQuestions && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.85)", backdropFilter: "blur(16px)" }} onClick={() => setNeedsQuestions(null)}>
          <div className="w-full max-w-sm rounded-2xl p-6 text-center" style={{ background: "linear-gradient(135deg, rgba(20,15,8,0.99), rgba(10,8,3,0.99))", border: "1px solid rgba(245,158,11,0.25)" }} onClick={e => e.stopPropagation()}>
            ...
            <p className="text-sm text-white/40 mb-6">You have <span className="font-black text-amber-400">{needsQuestions.question_count ?? 0}</span> questions. Need at least <span className="font-black text-white">5</span> to open registration.</p>
      ...
      {startError && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.85)", backdropFilter: "blur(16px)" }} onClick={() => setStartError(null)}>
          <div className="w-full max-w-sm rounded-2xl p-6 text-center" style={{ background: "linear-gradient(135deg, rgba(25,5,5,0.99), rgba(10,2,2,0.99))", border: "1px solid rgba(239,68,68,0.25)" }} onClick={e => e.stopPropagation()}>
            <MdWarning className="text-4xl text-red-400 mx-auto mb-4" />
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.85)", backdropFilter: "blur(16px)" }} onClick={() => setConfirmDelete(null)}>
          <div className="w-full max-w-sm rounded-2xl p-6 text-center" style={{ background: "linear-gradient(135deg, rgba(25,5,5,0.99), rgba(10,2,2,0.99))", border: "1px solid rgba(239,68,68,0.3)" }} onClick={e => e.stopPropagation()}>
            <MdDelete className="text-4xl text-red-400 mx-auto mb-4" />
      ...
      {confirmCancel && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.85)", backdropFilter: "blur(16px)" }} onClick={() => { setConfirmCancel(null); setCancelReason(""); }}>
          <div className="w-full max-w-sm rounded-2xl p-6 text-center" style={{ background: "linear-gradient(135deg, rgba(25,8,8,0.99), rgba(10,3,3,0.99))", border: "1px solid rgba(239,68,68,0.2)" }} onClick={e => e.stopPropagation()}>
            <MdWarning className="text-4xl text-red-400 mx-auto mb-4" />
