...
export const GamesTable: React.FC<GamesTableProps> = ({ games, clearFilters, refetch }) => {
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      setIsUpdating(true);
      await gameSessionService.retireGame(selectedGame.id, !selectedGame.isActive);
      refetch();
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        type={selectedGame?.isActive ? 'danger' : 'success'}
        title={`${selectedGame?.isActive ? 'Retire' : 'Activate'} Game?`}
        description={`Are you sure you want to ${selectedGame?.isActive ? 'retire' : 'activate'} "${selectedGame?.title}"? This will affect its visibility on the platform.`}
        confirmText={selectedGame?.isActive ? 'Retire Now' : 'Activate Now'}
      />
    </>
  );
};
