    ...
    <Table>
      <TableHeader>
        <TableRow className="border-none hover:bg-transparent">
          <TableHead className="px-8 py-5 text-[10px] uppercase tracking-[0.3em] font-black text-muted-foreground/40">Battle Arena</TableHead>
          <TableHead className="px-6 py-5 text-[10px] uppercase tracking-[0.3em] font-black text-muted-foreground/40 text-center">Protocol Score</TableHead>
          <TableHead className="px-6 py-5 text-[10px] uppercase tracking-[0.3em] font-black text-muted-foreground/40 text-center">Ranking</TableHead>
          <TableHead className="px-6 py-5 text-[10px] uppercase tracking-[0.3em] font-black text-muted-foreground/40 text-right">Loot Won</TableHead>
          <TableHead className="px-6 py-5 text-[10px] uppercase tracking-[0.3em] font-black text-muted-foreground/40">Timestamp</TableHead>
          <TableHead className="px-8 py-5 text-[10px] uppercase tracking-[0.3em] font-black text-muted-foreground/40 text-center">Status</TableHead>
        </TableRow>
      </TableHeader>
      <TableBody>
        {data.map((match) => (
          <TableRow key={match.id} className="group hover:bg-primary/5 transition-all border-border/10 rounded-2xl overflow-hidden">
            <TableCell className="px-8 py-7">
              <div className="flex items-center gap-5">
                <div className="w-14 h-14 rounded-2xl bg-muted group-hover:bg-primary/10 flex items-center justify-center text-primary transition-all duration-500 shadow-inner group-hover:rotate-12 border border-border/30">
            ...
            <TableCell className="px-6 py-7 text-right">
              <div className="flex flex-col items-end">
                <span className="text-foreground font-black text-xl leading-none">₦{match.winnings.toLocaleString()}</span>
...
export const FinancialFlow = ({ data }: { data: TransactionRecord[] }) => (
  <div className="p-4 md:p-10 animate-in fade-in slide-in-from-bottom-4 duration-500 overflow-x-auto no-scrollbar">
    <Table>
      <TableHeader>
        <TableRow className="border-none hover:bg-transparent">
          <TableHead className="px-8 py-5 text-[10px] uppercase tracking-[0.3em] font-black text-muted-foreground/40">Record Identity</TableHead>
          <TableHead className="px-6 py-5 text-[10px] uppercase tracking-[0.3em] font-black text-muted-foreground/40">Transaction Type</TableHead>
          <TableHead className="px-6 py-5 text-[10px] uppercase tracking-[0.3em] font-black text-muted-foreground/40 text-right">Ledger Amount</TableHead>
          <TableHead className="px-6 py-5 text-[10px] uppercase tracking-[0.3em] font-black text-muted-foreground/40">Access Channel</TableHead>
          <TableHead className="px-6 py-5 text-[10px] uppercase tracking-[0.3em] font-black text-muted-foreground/40">Registry Date</TableHead>
          <TableHead className="px-8 py-5 text-[10px] uppercase tracking-[0.3em] font-black text-muted-foreground/40 text-center">Outcome</TableHead>
        </TableRow>
      </TableHeader>
      <TableBody>
        {data.map((tx) => (
          <TableRow key={tx.id} className="group hover:bg-accent/5 transition-all border-border/10">
            <TableCell className="px-8 py-7 font-mono text-[10px] text-muted-foreground/60 font-black tracking-widest uppercase">REF-PZA-{tx.id}-X09</TableCell>
            <TableCell className="px-6 py-7">
            ...
            <TableCell className="px-6 py-7 text-right">
              <span className={`font-black text-lg ${tx.type === 'Deposit' ? 'text-emerald-500' : 'text-primary'}`}>₦{tx.amount.toLocaleString()}</span>
            </TableCell>
            <TableCell className="px-6 py-7">
              <div className="flex flex-col">
            ...
            <TableCell className="px-6 py-7 text-muted-foreground text-[10px] font-black uppercase tracking-widest">{tx.date}</TableCell>
            <TableCell className="px-8 py-7">
              <div className="flex justify-center">
                <span className={`text-[9px] font-black px-5 py-2 rounded-xl border ${
