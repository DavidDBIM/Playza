...
const SessionLeaderboardCard: React.FC<SessionLeaderboardCardProps> = ({ session, activeSessionId }) => {
  ...
  const statusColors = {
    ...
    Completed: 'bg-muted/10 text-muted-foreground border-border/30 shadow-muted/20'
  };

  return (
    <div id={`session-${session.id}`} className={`glass-card rounded-[1.5rem] overflow-hidden border border-border shadow-sm bg-white/30 dark:bg-black/10 transition-all hover:bg-white/50 dark:hover:bg-white/5 ${
      isExpanded ? 'border-primary/50 bg-primary/5' : ''
    } ${session.id === activeSessionId ? 'ring-2 ring-primary ring-offset-4 dark:ring-offset-background border-primary transition-all duration-500 scale-[1.01] shadow-xl shadow-primary/20 z-10' : ''}`}>
      <div 
      ...
      >
        <div className="flex flex-col md:flex-row md:items-center gap-6 flex-1">
          <div className="space-y-1">
            <h3 className="text-xl font-black text-foreground tracking-tight group-hover:text-primary transition-colors">{session.title}</h3>
            <div className="flex items-center gap-3">
              ...
              <span className="text-[9px] font-black uppercase text-muted-foreground/70 tracking-widest flex items-center gap-1">
          ...
          <div className="flex flex-wrap items-center gap-4 border-l border-border md:pl-6">
            <div className="flex flex-col">
              <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">Total Enrolled</span>
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 border-r border-border pr-4 mr-1">
            ...
            <Button variant="ghost" className="h-9 w-9 p-0 rounded-xl hover:bg-rose-500/10 hover:text-rose-500 transition-all text-muted-foreground/70" title="Reset Session Statistics">
              <MdDeleteForever className="text-lg" />
            </Button>
            <Button variant="ghost" className="h-9 w-9 p-0 rounded-xl hover:bg-primary/10 hover:text-primary transition-all text-muted-foreground/70" title="Full Session Details">
      ...
      {isExpanded && (
        <div className="p-4 md:p-6 pt-0 border-t border-border animate-in slide-in-from-top-4 duration-300">
          <div className="my-6">
