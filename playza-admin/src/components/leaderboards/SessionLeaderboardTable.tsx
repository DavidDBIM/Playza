...
const SessionLeaderboardTable: React.FC<SessionLeaderboardTableProps> = ({ leaderboard }) => {
  ...
  return (
    <div className="glass-card rounded-2xl overflow-hidden border border-border shadow-lg bg-white/50 dark:bg-black/20">
      <div className="overflow-x-auto no-scrollbar">
        <Table>
          <TableHeader className="bg-muted/30/50 dark:bg-white/5 border-b border-white/10">
            <TableRow className="hover:bg-transparent border-none">
              <TableHead className="px-6 py-4 text-[10px] text-muted-foreground uppercase tracking-widest pl-10 h-auto shadow-none border-none font-black">Rank</TableHead>
              <TableHead className="px-6 py-4 text-[10px] text-muted-foreground uppercase tracking-widest pl-15 h-auto shadow-none border-none font-black">Player</TableHead>
              <TableHead className="px-6 py-4 text-[10px] text-muted-foreground uppercase tracking-widest text-right pr-20 h-auto shadow-none border-none font-black">Score</TableHead>
              <TableHead className="px-6 py-4 text-[10px] text-muted-foreground uppercase tracking-widest text-right pr-15 h-auto shadow-none border-none font-black">Prize Estimate</TableHead>
            </TableRow>
          </TableHeader>
            ...
            {leaderboard.map((player) => (
              <TableRow 
                ...
                className={`group transition-all duration-300 hover:bg-primary/10 border-border/10 ${
                  ...
                  player.rank === 2 ? 'bg-muted/5 dark:bg-muted/20' : 
              ...
              >
                <TableCell className="px-6 py-4 pl-10 text-center relative w-20">
                  <div className={`text-xl font-bold flex items-center justify-center h-10 w-10 rounded-xl transition-transform group-hover:scale-110 shadow-sm border ${
                    ...
                    player.rank === 2 ? 'bg-muted/30 text-muted-foreground/70 border-border/30' :
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="space-y-0.5">
                      <p className={`font-black text-base tracking-tight transition-colors ${
                        player.rank === 1 ? 'text-amber-500' : 'text-foreground'
                      }`}>@{player.username}</p>
                      <p className="text-[9px] font-black uppercase text-muted-foreground/70 tracking-widest">ID: {player.userId}</p>
                    </div>
                  </div>
                </TableCell>
                <TableCell className="px-6 py-4 text-right pr-20 font-black text-foreground text-lg font-headline">
                  <div className="flex flex-col items-end">
                    ...
                    <span className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Pts Accumulated</span>
                  </div>
                </TableCell>
