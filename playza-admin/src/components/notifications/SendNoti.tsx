      ...
      <div className="grid grid-cols-12 gap-12">
        ...
        <div className="col-span-12 lg:col-span-7 space-y-8">
          ...
          <div className="glass-card rounded-[2rem] p-8 space-y-8 border border-slate-200 dark:border-white/5 bg-white/30 dark:bg-white/5 shadow-xl">
            <div className="grid grid-cols-2 gap-8">
              <div className="col-span-1">
                ...
                <div className="relative group">
                  ...
                  <MdExpandMore className="absolute right-5 top-4 text-zinc-500 text-xl pointer-events-none" />
                </div>
              </div>
              <div className="col-span-1">
                <label className="block text-[10px] font-black text-slate-500 dark:text-zinc-500 uppercase tracking-widest mb-3 ml-2">Priority Level</label>
            ...
            <div>
              <label className="block text-[10px] font-black text-slate-500 dark:text-zinc-500 uppercase tracking-widest mb-3 ml-2">Notification Title</label>
              <input 
                type="text"
                value={title}
            ...
            <div>
              <label className="block text-[10px] font-black text-slate-500 dark:text-zinc-500 uppercase tracking-widest mb-3 ml-2">Message Content</label>
              <textarea 
                value={content}
              ...
              {[
                ...
                <div key={i} className="p-5 rounded-2xl border border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-white/5 flex items-center justify-between cursor-pointer hover:border-primary/50 group transition-all shadow-md">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <span className="font-black text-slate-900 dark:text-white text-xs uppercase tracking-widest">{seg.label}</span>
                      <p className="text-[9px] font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-widest mt-0.5">{seg.sub}</p>
                    </div>
                  </div>
                  <div className="w-4 h-4 rounded-full border-2 border-slate-200 dark:border-white/20 group-hover:border-primary transition-colors"></div>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <label className="flex items-center gap-4 p-5 rounded-2xl border-2 border-primary bg-primary/5 cursor-pointer shadow-lg shadow-primary/5 group">
                ...
                <div className="flex-1">
                  <p className="font-black text-slate-900 dark:text-white text-xs uppercase tracking-widest">Send Now</p>
                  <p className="text-[9px] font-bold text-slate-500 dark:text-zinc-500 uppercase tracking-widest mt-1">Immediate delivery</p>
                </div>
                <MdBolt className="text-primary font-black text-xs uppercase tracking-widest group-hover:animate-bounce" />
              </label>
              <label className="flex items-center gap-4 p-5 rounded-2xl border-2 border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-white/5 cursor-pointer opacity-60 hover:opacity-100 transition-all group">
                <div className="w-5 h-5 rounded-full border-2 border-slate-400 dark:border-zinc-700 bg-white dark:bg-zinc-800"></div>
                <div className="flex-1">
                  <p className="font-black text-slate-400 dark:text-zinc-400 text-xs uppercase tracking-widest">Schedule</p>
                  <p className="text-[9px] font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-widest mt-1">Automated delivery</p>
                </div>
                <MdCalendarToday className="text-zinc-500 text-xl" />
              </label>
            </div>
          </div>
          ...
          <div className="flex gap-4 pt-4">
            ...
            <button className="flex-1 bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/5 text-slate-600 dark:text-white font-black uppercase tracking-widest py-5 rounded-2xl hover:bg-slate-200 dark:hover:bg-white/10 active:scale-[0.98] transition-all flex items-center justify-center gap-3 text-xs">
        ...
        <div className="col-span-12 lg:col-span-5 hidden lg:block">
          <div className="sticky top-28 space-y-8 animate-in slide-in-from-right-10 duration-700">
            <div className="glass-card rounded-[2.5rem] p-10 flex flex-col items-center border border-slate-200 dark:border-white/5 bg-white/50 dark:bg-white/5 shadow-2xl relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-primary/5 blur-3xl -z-10 rounded-full"></div>
              
              <div className="mb-8 flex items-center justify-between w-full">
                <h3 className="text-[10px] font-black text-slate-500 dark:text-zinc-500 uppercase tracking-widest">Live Preview</h3>
                <div className="flex gap-3">
                  <MdPhoneIphone className="text-primary text-xl" />
                  <MdTabletMac className="text-slate-300 dark:text-zinc-700 text-xl cursor-not-allowed" />
                </div>
              </div>

              {/* Phone Frame */}
              <div className="relative w-70 h-145 bg-slate-100 dark:bg-zinc-800 rounded-[3rem] border-10 border-slate-900 dark:border-zinc-950 shadow-2xl overflow-hidden ring-8 ring-slate-100/50 dark:ring-white/5">
                ...
                <div className="relative z-10 px-6 pt-12">
                  <div className="text-center mb-36 mt-10">
                    <p className="text-5xl font-black font-headline text-slate-900 dark:text-white/90 tracking-tighter">09:41</p>
                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-white/60 mt-1">Monday, October 24</p>
                  </div>

                  {/* Notification Card */}
                  <div className="bg-white/90 dark:bg-zinc-900/95 backdrop-blur-2xl p-4 rounded-2xl shadow-2xl border border-slate-200 dark:border-white/10 transform transition-all duration-300 scale-100 animate-pulse-subtle">
                    <div className="flex items-center gap-2 mb-2">
                      <div className="w-5 h-5 bg-primary rounded flex items-center justify-center text-[10px] text-white font-black">P</div>
                      <span className="text-[10px] font-black text-slate-900 dark:text-zinc-300 uppercase tracking-widest">Playza</span>
                      <span className="text-[10px] font-bold text-slate-400 dark:text-zinc-500 ml-auto lowercase">now</span>
                    </div>
                    <h4 className="text-sm font-black text-slate-900 dark:text-white mb-1 line-clamp-1">{title || 'Your Title'}</h4>
                    <p className="text-[10px] leading-relaxed text-slate-500 dark:text-zinc-400 font-bold line-clamp-3">
                ...
                {/* Notch */}
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-28 h-7 bg-slate-900 dark:bg-zinc-950 rounded-b-2xl z-20"></div>
                {/* Home Bar */}
                <div className="absolute bottom-2 left-1/2 -translate-x-1/2 w-24 h-1 bg-slate-900/10 dark:bg-white/10 rounded-full z-20"></div>
              </div>

              <div className="mt-8 w-full space-y-4">
                <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-zinc-500">
