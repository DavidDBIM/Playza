  ...
  return (
    <aside className="h-screen w-64 fixed left-0 top-0 overflow-y-auto bg-card border-r border-border shadow-2xl flex flex-col gap-2 py-8 z-50 transition-all duration-300 hidden lg:flex">
      <div className="px-8 mb-12">
        <h1 className="text-3xl font-black text-primary tracking-tighter font-heading italic leading-none">PLAYZA</h1>
        ...
        {[
          ...
          return (
            <Link
              key={item.label}
              to={item.path}
              className={`flex items-center gap-4 px-4 py-2.5 rounded-xl transition-all duration-200 group relative ${
                isActive
                  ? 'bg-primary text-primary-foreground shadow-lg shadow-primary/20'
                  : 'text-muted-foreground hover:text-foreground hover:bg-muted/50'
              }`}
            >
              <item.icon className={`text-lg transition-transform duration-300 group-hover:scale-110 ${isActive ? 'scale-110' : ''}`} />
